...
�?DeepBase 框架建立系统化的回归测试体系，覆盖已修复�?74 �?Bug，并集成�?CI 流程中�?
...
- [x] 1. 创建回归测试基础设施
  ...
  - [x] 1.2 创建回归测试基类 `Test.Regression.Base.pas`
    - 实现 TRegressionTestBase �?
    - 提供 GetBugNumber, GetBugDescription, GetFixDate 抽象方法
    - _Requirements: 1.2_
  - [x] 1.3 创建测试注册�?`RegressionTestRegistry.pas`
    - 定义 P0_TESTS �?P1_TESTS 常量数组
    - _Requirements: 1.3_
...
- [x] 2. 实现 P0 级别回归测试（Critical 安全漏洞�?
  - [x] 2.1 创建 BUG-058 测试：XOR加密安全缺陷
    ...
    - _Requirements: 2.1_
  - [x] 2.2 创建 BUG-062 测试：插件沙箱逃逸风�?
    - 验证插件路径验证
    - 验证数字签名验证机制
    - _Requirements: 2.2_
  - [x] 2.3 创建 BUG-063 测试：插件配置权限绕�?
    - 验证插件只能修改 Plugin. 前缀的配�?
    - _Requirements: 2.3_
  - [x] 2.4 创建 BUG-013 测试：支付模块RSA签名
    - 验证 RSA2-SHA256 签名实现
    - _Requirements: 2.4_
  - [x] 2.5 创建 BUG-035 测试：不安全随机数生�?
    - 验证使用安全随机数生成器
    - _Requirements: 2.5_
  - [x] 2.6 创建 BUG-007 测试：死锁风�?WhenReady
    - 验证回调中调�?DeepBase 功能不会死锁
    - _Requirements: 2.6_
  - [x] 2.7 创建 BUG-014 测试：微信支付签名验�?
    - 验证 RSA-SHA256 签名和验�?
    - _Requirements: 2.7_
  ...
  - [x] 2.9 创建 BUG-034 测试：硬编码密钥
    - 验证无硬编码默认密钥
    - _Requirements: 2.9_
  - [x] 2.10 创建 BUG-008 测试：UI线程竞态条�?
    - 验证 TThread.Synchronize 中的控件有效性检�?
    - _Requirements: 2.10_

- [x] 3. Checkpoint - P0 测试完成
  - 所�?P0 测试已创建完成，等待用户确认是否继续 P1 测试
...
- [x] 5. Checkpoint - P1 测试完成
  - 所�?P1 测试已创建完成，等待用户确认是否继续 CI 集成

- [x] 6. CI 集成
  - [x] 6.1 更新 `Scripts/run_tests.ps1`
    - 添加 `-Type Regression` 参数支持
    - 添加 `-Priority P0|P1|All` 过滤�?
...
- [x] 9. 将回归测试注册到主测试工�?
  - [x] 9.1 更新 `Tests/DeepBaseTests.dpr`
    - 添加回归测试单元引用
    - _Requirements: 4.1_

- [x] 10. Final Checkpoint - 全部完成
  - 回归测试系统实现完成
  - 22 个回归测试文件已创建�?0 �?P0 + 12 �?P1�?
  - CI 脚本已更新支持回归测�?
  - 覆盖率检查脚本已创建

## Notes

- 任务按优先级排序：P0 测试 > P1 测试 > CI 集成 > 覆盖�?> 文档
- 每个 Checkpoint 后暂停，等待用户确认
- P0 测试是最关键的安全相关测试，必须首先完成
- 并发相关测试�?.6）需要使�?`[RepeatTest(100)]` 属性多次运�?
