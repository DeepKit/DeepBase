# DeepBase Bug Fixes (归档卷 - 2025-11 ~ 2026-06-18 早期修复)

> 本卷归档 2026-06-18 三专家审阅及更早的全部 Bug 修复记录 (含 2025-11~12 Issues/性能/统计、2026-05 基础模块/Commerce 审计)。
> 近期修复 (REVIEW5 第一轮 / R2 第二轮 / R3 第三轮 + 06-21~28 OPT/EXP) 见 `bugfix.md`。
...
- 问题:
  ...
  - ✅ 增加源码/文档编码扫描门禁 (`Scripts/check_encoding.ps1`)：
    - 硬门禁 (InvalidUtf8 / Mojibake) 失败 CI；软门禁 (BOM 缺/多) 只告警。
    - `Scripts/encoding-allowlist.txt` 记录已知遗留破坏的 FMX 文件，降级为警告，避免阻塞新 PR。
    - `Scripts/run_tests.ps1` 新增 `-IncludeEncoding` 二级门禁 (同 `-IncludeStubApis` 模式)，CI 下触发。
  - ✅ 旧库一次性修复 migration (`Migrations/I18n/001_fix_bug276_seed_mojibake.up.sql`)：
    - UPDATE Languages.NativeName (zh-CN/zh-TW/ja-JP) 至正确 UTF-8。
...
- 修复记录:
  ...
  - 2026-06-22: 新增编码扫描门禁 + 遗留 FMX 文件 allowlist + run_tests.ps1 `-IncludeEncoding` 集成。
  - 2026-06-22: 新增 `Migrations/I18n/001_fix_bug276_seed_mojibake.up.sql` 旧库迁移。
- 状态: ✅ 全部完成（核心修复 + 测试 + 代码页 + 扫描门禁 + 旧库迁移）
...
- 修复计划:
  ...
  - ✅ Windows `ShareFileEx` 默认分支走 `ShellExecuteEx` + `lpVerb='share'` + `SEE_MASK_INVOKEIDLIST`；缺失文件 → `False`；`ShellExecuteEx` 失败 → 回退 `CopyToClipboard`；老版本 Windows 不支持 `share` 动词也能优雅降级。
  - ✅ iOS 框架头（AVFoundation / Photos / UserNotifications / Contacts / UIKit / Foundation）已接入，`CheckiOSPermission` / `RequestiOSPermission` 按 `ios.microphone / ios.camera / ios.photos / ios.notifications / ios.contacts` 键分发；真机路径以 `// TODO(on-device):` 留桩当前返回 `prUnsupported`，避免编译失败；`CheckPermissionEx` / `RequestPermissionEx` 分发链完整串联。
  ...
  - ⏳ iOS 真机路径：`AVAuthorizationStatus / PHAuthorizationStatus / UNAuthorizationStatus / CNAuthorizationStatus` 查询 + `UIActivityViewController` 分享 sheet；Android `FileProvider` 路径。需 Xcode + iOS 设备 / Android 真机 CI。
- 修复记录:
  ...
  - 2026-06-23: Windows `ShareFileEx` 改走 Shell `share` 动词；iOS 权限/分享桩 + 框架头接入；`TestFMXPlatformStandalone.dpr` 新增 `Test_ShareFileEx_MissingFile_ReturnsFalse`，17/17 PASS；DUnitX 端新增 4 个 delegate 链回归测试待合入 `DeepBaseTests.dpr`。
- 状态: 🟢 代码层修复完成（Windows Shell 路径 + iOS 框架头接入 + 桩 + 测试 17/17）；真机 CI（iOS / Android）留作后续
...
- 后续 (2026-06-23):
  - TDBVoiceProfileStorage (FireDAC) 实现完成：语音特征 BLOB + HMAC-SHA256 完整性保护 + ISO8601 日期 + owner_app 隔离。
  - 包重构：MFCC/DTW 迁入 DeepBaseSpeechCore；IVoiceProfileStorage 抽入 Contracts 单元，Persistence 包无反向依赖。
  - 11 个 DB 回归测试全部通过（0 failure，24 预存 CM 错误）。
- 状态: ✅ 全部完成
...
- 剩余:
  - [x] DUnitX 测试中 `Test_ConcurrentRegisterAndParse` 依赖 `System.Threading.TTask`，需要完整 CI 验证。
- 状态: ✅ 全部完成
  - 2026-06-24: REVIEW-P1-002 补完 — 新增 `Features/DeepBase.Speech.Intent.LLMBackend.pas` 官方生产适配器：`TIntentChatFunc` 注入式函数接口 + `BuildIntentPrompt` 纯函数 + `CreateIntentLLMBackend` 工厂；包边界不破坏（SpeechCore 不依赖 DeepBaseLLM）；9 个回归测试全部通过。
...
- 状态: ✅ 全部完成
  - 2026-06-24: REVIEW-P1-004 补完 — 新增 `ENotImplementedException` 继承自 `EInvalidOperationException`; FMX `Platform.pas` / `Theme.pas` 桩 IFDEF 分支添加 `raise ENotImplementedException`; 7 处 `TODO(BUG-xxx)` → `STUB(BUG-xxx)` 供 CI 扫描; `.github/workflows/delphi-ci.yml` 接入 `-IncludeStubApis -IncludeEncoding` 门禁; 3 个回归测试全部通过。
