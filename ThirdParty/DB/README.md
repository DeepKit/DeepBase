# UniBase Third-Party Database Drivers

第三方数据库驱动适配器，提供 PostgreSQL 和 MySQL 的高级功能支持。

## 文件列表

| 文件 | 说明 |
|------|------|
| `UniBase.DB.PostgreSQL.pas` | PostgreSQL 驱动适配器 |
| `UniBase.DB.MySQL.pas` | MySQL 驱动适配器 |
...
```pascal
// 简单查询
TPgFullTextQuery.PlainTo('database optimization');
...
| 模式 | 说明 |
|------|------|
| `ftNaturalLanguage` | 自然语言模式，按相关性排序 |
| `ftBooleanMode` | 布尔模式，支持 +, -, *, "", () |
...
1. 需要安装对应的数据库客户端库
2. PostgreSQL 需要 libpq.dll
3. MySQL 需要 libmysql.dll
4. 建议生产环境启用 SSL
