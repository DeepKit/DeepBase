...
## 2026-08-29 ~ 2026-08-30 VCL HB 视觉基础设施审计、修复与组件补齐（BUG-449 ~ BUG-471 / FEAT-HB-001 ~ FEAT-HB-005）

> **审计范围**：`VCL/DeepBase.VCL.HB.*.pas` 全 22 个源码文件 + `Core/DeepBase.HB.*.pas` 10 个核心类型单元。  
> **关联工单**：`WO-20260829-0230`、`WO-20260830-补修`、`WO-20260830-最终验证`、`WO-20260830-文档整理`、`WO-20260830-003`、`WO-20260830-HB-交付报告纠偏`、`WO-20260830-004`。  
> **最终状态**：全部已修复并交付验收 ✅（编译门禁 32/32 单元 0 Error 0 Warning；单测门禁 `Test.DeepBase.HB.Suite` 25/25 通过；全模块 `-Module HB` 61/61 通过）。

---

### 一、P1 必修稳定性缺陷（9项 · 全部已修复 ✅）

- **BUG-449** `Dialogs.pas:517` — ShowXxx 通用流程在非 Prompt 模式下点确定按钮直接访问 `EdtInput.Text`，而 `EdtInput` 仅在 `dkPrompt/dkPromptReason` 时创建，触发 AV 崩溃。
  - **修复**: 增加空指针与模式守卫：`if Assigned(EdtInput) and (AOptions.Kind in [dkPrompt, dkPromptReason]) then AInputValue := EdtInput.Text;`。
  - **状态**: ✅ 已修复（WO-20260829-0230 / DUnitX 回归覆盖）。

- **BUG-450** `Tray.pas:636-640` — `THbTrayIcon.Destroy` 析构函数中显式调用 `FTrayIcon.Free; FMenu.Free;`，与构造时以 `Self` 作为 `Owner` 的 TComponent 机制冲突，导致组件销毁时二次释放崩溃。
  - **修复**: 移除析构中的显式 Free，完全交由 TComponent Owner 链自动回收。
  - **状态**: ✅ 已修复（WO-20260829-0230 / DUnitX `Test.DeepBase.HB.Tray` 覆盖）。

- **BUG-451** `Terminal.pas:626` — `THbToast` 既以 `FHostControl` 为 Owner 创建，又被添加到 `FActiveToasts` 列表管理，宿主销毁时触发双释放。
  - **修复**: 创建时设 `Owner = nil`，生命周期由 `FActiveToasts` 独占管理。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-452** `Controls.pas:673` — `THbButton.Paint` 中 `bkDanger` 分支缺失 Hover / Pressed / Disabled 状态判断与取色逻辑，危险按钮在悬停和按下时无视觉反馈。
  - **修复**: 补齐 `bkDanger` 下各状态机色彩映射：`Tokens.Danger`, `Tokens.DangerHover`, `Tokens.DangerPressed` 等。
  - **状态**: ✅ 已修复（WO-20260829-0230 / `Test_HB_Button_Danger_Token_Alignment`）。

- **BUG-453** `Controls.pas:880` — `THbDualButton.Paint` 把悬停视觉反馈条件错误写成 `IsHover and IsPressed`，导致纯鼠标悬停时没有任何视觉反馈。
  - **修复**: 拆分为独立的三态分层判断：Hover、Pressed、Normal 各自独立取色与绘制。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-454** `Controls.pas:1805` — `THbSectionHeader` 点击命中测试区域固定为 80px 宽，但实际 Paint 按照容器 50% 宽度绘制热区，宽容器下点击右半部分无响应。
  - **修复**: 统一命中测试区域与绘制区域宽度计算，均采用 `Width div 2` 动态尺寸。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-455** `Controls.pas:1471` — `THbProgressRing.Paint` 当 `Thickness > Min(Width, Height)/2` 时，`RectF.Inflate(-T, -T)` 导致矩形坐标反向，GDI+ 抛出异常。
  - **修复**: 入口增加尺寸约束钳制：`FStrokeWidth := Min(FStrokeWidth, Min(Width, Height) div 4);`。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-456** `Cards.pas:447` — `THbStatBig.Paint` 完全未绘制 `FTrend` 与 `FTrendText` 属性，调用方赋值后界面无视觉反馈。
  - **修复**: 在 Paint 末尾增加趋势指示渲染分支，绘制 ▲/▼ 矢量箭头与趋势说明文本。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-457** `Cards.pas:823` — `THbListRow.MouseDown` 直接复用上一次 `MouseMove` 缓存的 `FActionHoverPart`，若鼠标未移动直接瞬间点按则判定错位。
  - **修复**: `MouseDown` 内依据当前点击的 X/Y 坐标实时计算 `FActionPressPart` 与 `FActionHoverPart`。
  - **状态**: ✅ 已修复（WO-20260829-0230 / WO-20260830-补修）。

---

### 二、P2 性能与安全缺陷（6项 · 全部已修复 ✅）

- **BUG-458** `Cards.pas:708 / Controls.pas:603` — `THbListRow.Paint` 与 `THbButton.Paint` 每次 Paint 重复创建 ~9 个 GDI+ 对象（FontFamily/Font/StringFormat/SolidBrush/Pen 等），高频滚动列表时产生严重 GC 压力与抖动。
  - **修复**: 将 FontFamily、Font、StringFormat 等重度对象提取为类成员缓存，仅在主题切换或 DPI 变化时重新构建。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-459** `Controls.pas:1444 / 1718` — `THbProgressRing` 与 `THbSkeleton` 使用 30ms 高频定时器驱动 `Invalidate`，动画帧率达 33fps 产生无谓 CPU 占用。
  - **修复**: 定时器周期调整为 60ms 平滑降频，降低 50% CPU 占用且视觉完全流畅。
  - **状态**: ✅ 已修复（WO-20260829-0230 / WO-20260830-补修）。

- **BUG-460** `CommandPalette.pas` — `RebuildFilteredList` 每次输入均做全量字符串匹配与排序，无防抖无缓存，长列表输入卡顿。
  - **修复**: 增加 100ms 输入防抖定时器，并对选项文本加入小写缓存优化。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-461** `Terminal.pas` — StreamBlock 流式块的 60ms 合并定时器在状态进入 `ssCommitted` 之后仍持续触发空 `Invalidate` 轮询。
  - **修复**: 进入 `ssCommitted` 状态后立即停用定时器：`FMergeTimer.Enabled := False;`。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-468** `ShareCard.pas:82` — `MaskSensitiveText` 仅针对连续 ≥3 位的纯数字做掩码，遇到空格分割的银行卡号或混合敏感信息（如 `"123 456"`）出现漏掩。
  - **修复**: 重构掩码引擎，采用多模式正则全面覆盖手机号、身份证号、邮箱与格式化卡号。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-469** `PageControl.pas:220-230` — `MouseMove` 悬停状态变化时对整个 PageControl 触发全量 `Invalidate`，引起整个页面控件区域闪烁。
  - **修复**: 改用 Windows API `InvalidateRect`，仅对发生状态变化的旧 Tab 与新 Tab 矩形区域做局部无效化。
  - **状态**: ✅ 已修复（WO-20260830-补修）。

---

### 三、P3 美观度与高分屏适配（8项 · 全部已修复 ✅）

- **BUG-462** `Dialogs.pas` — 整个对话框框架内按钮尺寸、外边距、文本位置大量硬编码（16/48/86/528），未接入 `ScaleDIP` / `CurrentPPI`，在高分屏（150%/200%）下文字截断、按钮重叠。
  - **修复**: 全面重构布局逻辑，所有像素常数全部接入 `ScalePixels` / `ScaleDIP`，自适应高 DPI 缩放。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-463** `Voice.pas / Gate.pas` — `FRowHeightCollapsed=42 / FRowHeightExpanded=130` 等高度常量硬编码，未随 PPI 缩放。
  - **修复**: 绑定 Design Tokens 与 DPI 缩放管线，自适应缩放。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-464** `Cards.pas:1285 / Controls.pas:1653` — Avatar 与 ListRow 的 `GetSeedColor` 使用 `mod 4` 仅产生 4 种颜色，同列表高频撞色。
  - **修复**: 抽取公共哈希调色板函数 `GetDeterministicColor`，扩展至 12 种语义和谐的哈希色系。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-465** `Cards.pas / Terminal.pas` — `THbToast` 与 `THbEmptyState` 使用 Emoji 字符（"✓ ✕ ℹ ⚠"）作为状态图标，不同系统字体回退后出现豆腐块或色差。
  - **修复**: 改为 GDI+ 矢量路径自绘高质量圆角图标，保证跨 Windows 版本视觉一致性。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-466** `Controls.pas:1028` — `THbChip` 的关闭按钮采用追加 `'  ✕'` 字符实现，无独立鼠标悬停热区与反馈。
  - **修复**: 独立绘制矢量圆形关闭按钮，支持独立的鼠标 Hover 高亮反馈与点击回调。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-467** `Theme.pas + Palettes.pas` — VCL 层主题单元仅作为 Core 层的类型转发壳，未提供运行时覆盖与深浅色切换能力。
  - **修复**: 引入 `THbTheme.RegisterOverride` 钩子机制与 `ApplyTheme` 运行时切换广播。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-470** `NavTree.pas:340` — NavTree 在 Mini Rail 折叠态下节点文本直接截断且无图标指示。
  - **修复**: 增加折叠态首字母缩写徽章与悬浮 Tooltip 气泡提示。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

- **BUG-471** `ShareCard.pas` — 字体硬编码为 `'Segoe UI'` / `'Consolas'`，未接入 `Tokens.FontFamily`。
  - **修复**: 全面绑定 `Tokens.FontFamily` 与 `Tokens.FontFamilyMono`。
  - **状态**: ✅ 已修复（WO-20260829-0230）。

---

### 四、HB 视觉系统缺失组件补齐（5项新特性 · 全部已交付 ✅）

- **FEAT-HB-001** `VCL\DeepBase.VCL.HB.Text.pas` — `THbText` / `THbLabel` 语义化 Typography 排版文本标签。
  - **能力**: 支持 10 种排版角色（`trBody`, `trMuted`, `trPrimary`, `trSecondary`, `trSuccess`, `trWarning`, `trDanger`, `trInfo`, `trHeading`, `trSubheading`, `trCaption`），支持 WordWrap、多行自适应、水平/垂直对齐、Design Tokens 颜色/字体绑定及高 DPI 缩放。
  - **状态**: ✅ 已交付（WO-20260830-003 / `Test_HbText_Roles_And_Tones`）。

- **FEAT-HB-002** `VCL\DeepBase.VCL.HB.Inputs.pas` — 现代矢量圆角输入控件集。
  - **能力**: 
    - `THbEdit`：矢量圆角单行输入框，内置占位提示（Placeholder）、一键清除按钮（ClearButton）、焦点光环（FocusRing）、只读与密码模式。
    - `THbComboBox`：矢量圆角下拉框，自定义 Chevron 箭头与主题色下拉菜单。
    - `THbCheckBox`：矢量圆角复选框，平滑 Checkmark 矢量勾选与 Token 语义着色。
    - `THbToggleSwitch`：行内胶囊启停开关，即点即生效，支持 ON/OFF 状态文本与主题主色渲染。
    - `THbThemeSelector`：通用主题切换下拉控件，自动枚举加载可用主题并在切换时自动调用 `THbTheme.ApplyTheme` 广播 `WM_HB_THEME_CHANGED`。
  - **状态**: ✅ 已交付（WO-20260830-003 / `Test_HbCheckBox_*`, `Test_HbToggleSwitch_*`, `Test_HbEdit_*`, `Test_HbComboBox_*`, `Test_HbThemeSelector_*`）。

- **FEAT-HB-003** `VCL\DeepBase.VCL.HB.Status.pas` — `THbStatusDot` 状态指示灯。
  - **能力**: 支持五态映射（`sdSuccess`, `sdDanger`, `sdWarning`, `sdMuted`, `sdInfo`），支持 60ms 呼吸脉冲光晕微动效（`Pulse: Boolean`），可选伴随文本标签，替代原生 TShape。
  - **状态**: ✅ 已交付（WO-20260830-003 / `Test_HbStatusDot_States_And_Pulse`）。

- **FEAT-HB-004** `VCL\DeepBase.VCL.HB.Glass.pas` — `THbGlassPanel` 磨砂悬浮面板。
  - **能力**: Windows 11 Fluent 亚克力/磨砂半透明毛玻璃背景（Acrylic Alpha 混合），多层软阴影高斯投影（`DropShadow: Boolean`），支持 `FadeIn` / `FadeOut` 微过渡动效与子控件容器化（`csAcceptsControls`）。
  - **状态**: ✅ 已交付（WO-20260830-003 / `Test_HbGlassPanel_Opacity_And_Transitions`）。

- **FEAT-HB-005** `VCL\DeepBase.VCL.HB.Grid.pas` + `Core\DeepBase.HB.Grid.Types.pas` — `THbDataGrid` 行内控件扩展。
  - **能力**: 新增 `gctToggleSwitch` 与 `gctCheckbox` 列类型支持，提供 `OnGetCellBool` 与 `OnCellToggle` 实时点击回调，实现任务与纳管列表的行内即点即生效。
  - **状态**: ✅ 已交付（WO-20260830-003 / `Test_HbDataGrid_Inline_Toggle_And_Checkbox`）。
...
1. **Win64 编译器门禁**：
   - 针对全量 32 个 HB 源码与测试单元执行 `dcc64 -Q -B` 编译，**32/32 全部 0 Error 0 Warning 通过**。
   - 证据文件：`docs/evidence-dcc64-hb-32units.txt`。

2. **DUnitX 单元测试门禁**：
   - **HB 核心控件专项套件** (`Test.DeepBase.HB.Suite`)：实测 **25 / 25 全部通过**（0 失败，0 错误，0 泄漏）。
   - **全量 HB 视觉子系统模块** (`-Module HB` / 6 Fixtures)：实测 **61 / 61 全部通过**（0 失败，0 错误，0 泄漏）。

---

## 2026-09-02 WO-20260902-001 框架审计 Top10 修复（Core + Persistence）

> 来源: `CodeReview/20260902-Framework-Audit.md` · 工单: `docs/WO-20260902-001-开发甲-框架审计P0修复工单.md`
> 状态: 代码已落地工作区；全量回归证据与 `RegressionTestRegistry` 登记待闭环

### P0 — 并发 / 生命周期 / UAF

#### BUG-334 DB.Pool Shutdown 超时后仍释放 csInUse 连接 ✅
- **文件**: `Persistence/DeepBase.DB.Pool.pas` (~1090)
- **问题**: Drain 超时后 `FPool.Clear` 释放仍在借出中的连接 → UAF/堆损坏
- **修复**: 仅移除非 `csInUse` 条目；in-use 连接 intentional leak + DEBUG 日志
- **回归**: `Tests/Regression/Test.Regression.BUG334_PoolShutdownInUse.pas`

#### BUG-335 DB.Guardian 瞬态 Open 失败误判 corrupt ✅
- **文件**: `Persistence/DeepBase.DB.Guardian.pas` (~494)
- **问题**: 任意 Open 异常走 quarantine+备份恢复 → busy/路径错误等瞬态故障静默回滚
- **修复**: `ClassifyOpenError` 分 structural/transient；仅 structural 走 `QuarantineAndRecover`
- **回归**: `Tests/Regression/Test.Regression.BUG335_GuardianTransientOpen.pas`

#### BUG-326 扩展 Scheduler FRunningITask 提前置 nil ✅
- **文件**: `Core/DeepBase.Scheduler.pas` (~1035, ~1084)
- **问题**: 注释要求回调后清 nil，实际在 OnCompleted/OnFailed 之前置 nil → Cleanup 并发 UAF
- **修复**: 回调在锁外执行；`FRunningITask:=nil` 移到回调返回后重新进锁清除
- **回归**: `Tests/Regression/Test.Regression.BUG326_SchedulerCallbackSafety.pas::Test_OnCompleted_CleanupDuringCallback_TaskRefSafe`

#### BUG-320 扩展 FileWatcher Debounce 裸 Guard 捕获 ✅
- **文件**: `Core/DeepBase.FileWatcher.pas` (~868)
- **问题**: `HandleDebounce` 匿名 TTask 捕获 `TFileWatcherGuard` 裸引用 → Watcher 释放后 UAF
- **修复**: 同 `NotifyChange` 范本，改用 `LGuard: IInterface` 持有引用计数
- **回归**: `Tests/Regression/Test.Regression.BUG320_FileWatcherLifecycle.pas::Test_DestroyWithDebounce_NoCrash`

#### BUG-336 WorkerQueue Stop(False) 不 WaitFor 即 Clear ✅
- **文件**: `Core/DeepBase.WorkerQueue.pas` (~2158)
- **问题**: Terminate 后不 WaitFor 即 `FWorkers.Clear`（owns 线程）→ 工作线程 UAF
- **修复**: 两分支均 Terminate → WaitFor → Free；列表先 Clear 再逐个 join
- **回归**: `Tests/Regression/Test.Regression.BUG336_WorkerQueueStopWait.pas`
- **备注**: `AWaitForCompletion` 参数现为 dead code，待后续 API 清理或恢复队列 drain 语义

#### BUG-340 PluginManager UnloadBPL 后接口悬垂 ✅
- **文件**: `Core/DeepBase.PluginManager.pas` (~804)
- **问题**: 本地 `PluginIntf` 跨过 `UnloadBPL` 存活 → 已卸载 BPL vtable UAF
- **修复**: Finalize → 从 FPlugins/FLoadOrder 移除 → `PluginIntf:=nil` → 最后 UnloadBPL
- **回归**: `Tests/Regression/Test.Regression.BUG340_PluginUnloadOrder.pas`

### P0 — 功能 / 数据正确性

#### BUG-327 扩展 KeyManager GCM 空明文 off-by-one ✅
- **文件**: `Core/DeepBase.KeyManager.pas` (~1076)
- **问题**: `(Length > 29)` 严格大于，空明文 GCM 包恰 29 字节误走 legacy CBC
...
#### BUG-337 Authorization UpdateUser/UpdateRole 内存态 stale ✅
- **文件**: `Core/DeepBase.Authorization.pas` (~1094, ~1243)
- **问题**: 仅写库不更新 FUsers/FRoles → GetUser/HasPermission 读到旧值
- **修复**: Save 成功后同步更新字典内 Live 对象字段（与 GetUser 克隆策略一致）
- **回归**: `Tests/Regression/Test.Regression.BUG337_AuthorizationUpdateSync.pas`

#### BUG-338 DoQry Bind 前静默 Trim ✅
- **文件**: `Persistence/DeepBase.DB.DoQry.pas` (~853)
- **问题**: 字符串参数 `Trim` 后绑定，首尾空格被静默删除
- **修复**: UUID 探测用局部 `STrim`；`P.AsWideString` 绑定原始值
- **回归**: `Tests/Regression/Test.Regression.BUG338_DoQryBindTrim.pas`

#### BUG-339 DoQry Sweep 不检查 InUseCount ✅
- **文件**: `Persistence/DeepBase.DB.DoQry.pas` (~1846)
