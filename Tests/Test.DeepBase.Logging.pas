...
{*******************************************************************************
  ...
  Notes:
  - TDeepBaseLogger writes asynchronously via a background thread.
...
type
  [TestFixture]
  TTestDeepBaseLogging = class
  private
    FLog: TDeepBaseLogger;
    FLogDir: string;
    FDatePrefix: string;
...
procedure TTestDeepBaseLogging.CleanupTodayLogs;
var
  Files: TArray<string>;
  F: string;
begin
...
function TTestDeepBaseLogging.FindTodayLogFiles: TArray<string>;
begin
...
function TTestDeepBaseLogging.WaitForAnyTodayFileContains(const SubStr: string; TimeoutMs: Integer): Boolean;
var
  SW: TStopwatch;
  Files: TArray<string>;
  F: string;
...
procedure TTestDeepBaseLogging.Setup;
begin
  ...
  FLog := TDeepBaseLogger.Create('');
  FLog.StorageMode := lsmFile;
  FLog.LogFormat := lfText;
  FLog.MinLevel := llDebug;
end;

procedure TTestDeepBaseLogging.TearDown;
begin
  FreeAndNil(FLog);
  CleanupTodayLogs;
end;

procedure TTestDeepBaseLogging.Test_Info_WritesToFile;
var
  Msg: string;
begin
...
procedure TTestDeepBaseLogging.Test_InfoFmt_WritesFormattedText;
var
  MsgPart: string;
begin
...
procedure TTestDeepBaseLogging.Test_ClearOldLogs_DoesNotRaise;
begin
  try
...
procedure TTestDeepBaseLogging.Test_ThreadSafety_ConcurrentWrites_DoesNotRaise;
const
  THREADS = 8;
  PER_THREAD = 200;
var
...
procedure TTestDeepBaseLogging.Test_StorageInjection_DelegatesDbWriteAndQuery;
var
  InjectedStorageObj: TInMemoryLogStorage;
  InjectedStorage: ILogStorage;
  InjectedLogger: TDeepBaseLogger;
  Msg: string;
  SW: TStopwatch;
begin
  ...
  try
    InjectedLogger := TDeepBaseLogger.Create(':memory:');
    try
    ...
    TDeepBaseLogger.SetStorageFactory(TFunc<string, ILogStorage>(nil));
  end;
end;

initialization
  TDUnitX.RegisterTestFixture(TTestDeepBaseLogging);
