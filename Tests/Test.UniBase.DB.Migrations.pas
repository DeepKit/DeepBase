unit Test.UniBase.DB.Migrations;
...
begin
  ...
  GuidText := StringReplace(GuidText, '}', '', [rfReplaceAll]);
  FTempDir := TPath.Combine(TPath.GetTempPath, 'UniBase_Migrations_' + GuidText);
...
begin
  ...
  Assert.AreEqual(2,
    ScalarInt('SELECT COUNT(*) FROM unibase_schema_migrations'));
end;
