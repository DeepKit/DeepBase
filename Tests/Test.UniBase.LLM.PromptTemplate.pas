{ ============================================================================
  Test.UniBase.LLM.PromptTemplate - Prompt Template Unit Tests
  
  说明: 测试 LLM Prompt 模板管理功能
  ============================================================================ }

unit Test.UniBase.LLM.PromptTemplate;
...
begin
  ...
  try
    Vars.Add('name', 'Alice');
    Vars.Add('place', 'UniBase');
    
    Result := T.RenderUserPrompt(Vars);
    Assert.AreEqual('Hello Alice, welcome to UniBase!', Result);
  finally
    Vars.Free;
  end;
end;
