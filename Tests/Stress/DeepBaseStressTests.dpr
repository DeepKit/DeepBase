{ ============================================================================
  DeepBase Stress Tests
  
  Comprehensive stress testing suite for DeepBase framework.
  Tests high-concurrency, large-scale operations, and long-running stability.
  
  Usage:
    DeepBaseStressTests.exe [options]
  ...
  Examples:
    DeepBaseStressTests.exe -suite=all -duration=300 -threads=20
    DeepBaseStressTests.exe -suite=config -output=report.html -format=html
    DeepBaseStressTests.exe -quick -verbose
  ============================================================================ }

program DeepBaseStressTests;
...
uses
  ...
  System.IOUtils,
  Winapi.Windows,
  DeepBase.StressTest in 'DeepBase.StressTest.pas',
...
begin
  try
    ...
    // Initialize DeepBase
    WriteLn('Initializing DeepBase...');
    try
      if not DeepBase.Manager.DeepBase.InitializeWithDB(
        ExtractFilePath(ParamStr(0)) + 'stress_test.db') then
      begin
        WriteLn('Warning: DeepBase initialization returned false: ' + 
          DeepBase.Manager.DeepBase.LastError);
      end
      else
        WriteLn('DeepBase initialized successfully.');
      WriteLn;
    except
      on E: Exception do
      begin
        WriteLn('Failed to initialize DeepBase: ' + E.Message);
