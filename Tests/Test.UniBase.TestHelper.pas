...
{ TTestUniBaseTestHelper }

procedure TTestUniBaseTestHelper.Setup;
begin
  FOriginalSnapshotPath := TUniBaseTestHelper.SnapshotPath;
  FTempSnapshotPath := TPath.Combine(TPath.GetTempPath, 'UniBaseTestHelperTests');
  TUniBaseTestHelper.Initialize(nil, FTempSnapshotPath);
...
procedure TTestUniBaseTestHelper.TearDown;
begin
  ...
  TUniBaseTestHelper.SnapshotPath := FOriginalSnapshotPath;
end;

procedure TTestUniBaseTestHelper.ButtonClick(Sender: TObject);
begin
  FClicked := True;
end;

procedure TTestUniBaseTestHelper.Test_SnapshotPath_Is_Set_And_Directory_Created;
begin
  Assert.AreEqual(FTempSnapshotPath, TUniBaseTestHelper.SnapshotPath);
  Assert.IsTrue(TDirectory.Exists(FTempSnapshotPath), 'Snapshot directory should exist after Initialize');
end;

procedure TTestUniBaseTestHelper.Test_CaptureFormState_Includes_Form_And_Control_Properties;
var
  JsonStr: string;
  RootObj, ControlObj, ChildObj: TJSONObject;
  Value: TJSONValue;
begin
  JsonStr := TUniBaseTestHelper.CaptureFormState(FForm);
...
procedure TTestUniBaseTestHelper.Test_SaveSnapshot_And_GetSnapshot_File_Mode;
var
  SnapshotName: string;
  JsonStr: string;
  FileName: string;
begin
  ...
  TUniBaseTestHelper.SaveSnapshot(SnapshotName, FForm, False {SaveScreenshot});

  // file should exist
  FileName := TPath.Combine(TUniBaseTestHelper.SnapshotPath, SnapshotName + '.json');
  Assert.IsTrue(TFile.Exists(FileName), 'Snapshot JSON file should be created');

  JsonStr := TUniBaseTestHelper.GetSnapshot(SnapshotName);
  Assert.IsNotEmpty(JsonStr, 'GetSnapshot should return JSON content');
end;

procedure TTestUniBaseTestHelper.Test_VerifySnapshot_Success_When_State_Unchanged;
var
  SnapshotName: string;
  ResultRec: TSnapshotVerifyResult;
begin
  SnapshotName := 'Test_VerifySnapshot_Success';
  TUniBaseTestHelper.SaveSnapshot(SnapshotName, FForm, False);

  ResultRec := TUniBaseTestHelper.VerifySnapshot(SnapshotName, FForm);
...
procedure TTestUniBaseTestHelper.Test_VerifySnapshot_Returns_NotFound_When_Snapshot_Missing;
var
  ResultRec: TSnapshotVerifyResult;
  Name: string;
begin
  Name := 'NonExistentSnapshot_' + TGUID.NewGuid.ToString;
  ResultRec := TUniBaseTestHelper.VerifySnapshot(Name, FForm);
...
procedure TTestUniBaseTestHelper.Test_VerifySnapshot_Detects_Differences_And_Populates_Diffs;
var
  SnapshotName: string;
...
procedure TTestUniBaseTestHelper.Test_ListSnapshots_Includes_Saved_Snapshot;
var
  SnapshotName: string;
  Names: TArray<string>;
  Name: string;
...
begin
  ...
  Names := TUniBaseTestHelper.ListSnapshots;
  Found := False;
...
procedure TTestUniBaseTestHelper.Test_DeleteSnapshot_Removes_JSON_And_PNG;
var
  SnapshotName: string;
...
begin
  ...
  JsonFile := TPath.Combine(TUniBaseTestHelper.SnapshotPath, SnapshotName + '.json');
  PngFile := TPath.Combine(TUniBaseTestHelper.SnapshotPath, SnapshotName + '.png');
  ...
  TUniBaseTestHelper.DeleteSnapshot(SnapshotName);
...
procedure TTestUniBaseTestHelper.Test_GetControlValue_And_AssertValue_Work_For_Common_Controls;
begin
  ...
  TUniBaseTestHelper.AssertValue(FEdit, 'John');
  TUniBaseTestHelper.AssertValue(FCheckBox, 'True');

  // and should raise when values differ for checkbox
  Assert.WillRaise(
    procedure
    begin
      TUniBaseTestHelper.AssertValue(FCheckBox, 'False');
    end,
    ETestAssertionFailed
  );
end;

procedure TTestUniBaseTestHelper.Test_AssertVisible_And_AssertEnabled_Raise_When_Conditions_Not_Met;
begin
  // visible
  ...
  Assert.WillRaise(
    procedure
    begin
      TUniBaseTestHelper.AssertVisible(FButton);
    end,
    ETestAssertionFailed
  );
  ...
  Assert.WillRaise(
    procedure
    begin
      TUniBaseTestHelper.AssertEnabled(FButton);
    end,
    ETestAssertionFailed
  );
end;

procedure TTestUniBaseTestHelper.Test_FindControl_And_FindControlByClass_Work_Correctly;
var
  Ctrl: TControl;
  EditCtrl: TEdit;
begin
  Ctrl := TUniBaseTestHelper.FindControl(FForm, 'btnOK');
  ...
  EditCtrl := TUniBaseTestHelper.FindControlByClass<TEdit>(FForm);
...
procedure TTestUniBaseTestHelper.Test_FindAllControlsByClass_Returns_All_Instances;
var
  Buttons: TArray<TButton>;
begin
  Buttons := TUniBaseTestHelper.FindAllControlsByClass<TButton>(FForm);
...
procedure TTestUniBaseTestHelper.Test_SimulateSelect_ByIndex_And_ByText_Updates_Index;
begin
  // ComboBox by index
  TUniBaseTestHelper.SimulateSelect(FComboBox, 1);
  Assert.AreEqual(1, FComboBox.ItemIndex, 'ComboBox index should be updated');

  // ListBox by text
  TUniBaseTestHelper.SimulateSelect(FListBox, 'Two');
  Assert.AreEqual(FListBox.Items.IndexOf('Two'), FListBox.ItemIndex, 'ListBox index should be updated by text');

  // ListView by index
  TUniBaseTestHelper.SimulateSelect(FListView, 0);
  Assert.AreEqual(0, FListView.ItemIndex, 'ListView index should be updated');
end;

procedure TTestUniBaseTestHelper.Test_SimulateCheck_Updates_Checked_State;
begin
  TUniBaseTestHelper.SimulateCheck(FCheckBox, False);
  Assert.IsFalse(FCheckBox.Checked, 'CheckBox should be unchecked');

  TUniBaseTestHelper.SimulateCheck(FCheckBox, True);
  Assert.IsTrue(FCheckBox.Checked, 'CheckBox should be checked');
end;

procedure TTestUniBaseTestHelper.Test_SimulateInput_Updates_Text;
begin
  TUniBaseTestHelper.SimulateInput(FEdit, 'Alice');
  Assert.AreEqual('Alice', FEdit.Text, 'SimulateInput should update edit text');
end;

procedure TTestUniBaseTestHelper.Test_SimulateClick_And_DoubleClick_Do_Not_Raise;
begin
  // If these raise any exception, the test will fail automatically
  TUniBaseTestHelper.SimulateClick(FButton);
  TUniBaseTestHelper.SimulateDoubleClick(FButton);
end;

procedure TTestUniBaseTestHelper.Test_SimulateKeyPress_And_MouseMove_Do_Not_Raise;
var
  OldPos: TPoint;
begin
  GetCursorPos(OldPos);
  // If these raise any exception, the test will fail automatically
  TUniBaseTestHelper.SimulateKeyPress(FEdit, Ord('A'));
  TUniBaseTestHelper.SimulateMouseMove(FButton, 5, 5);
  // best-effort: restore cursor position
  SetCursorPos(OldPos.X, OldPos.Y);
end;

procedure TTestUniBaseTestHelper.Test_CaptureScreenshot_And_SaveScreenshotToFile_Work;
var
  Bitmap: TBitmap;
  PngFile: string;
begin
  Bitmap := TUniBaseTestHelper.CaptureScreenshot(FForm);
  try
  ...
  PngFile := TPath.Combine(TUniBaseTestHelper.SnapshotPath, 'Test_Screenshot.png');
  if TFile.Exists(PngFile) then
    TFile.Delete(PngFile);

  TUniBaseTestHelper.SaveScreenshotToFile(FForm, PngFile);
  Assert.IsTrue(TFile.Exists(PngFile), 'SaveScreenshotToFile should create PNG file');
end;

procedure TTestUniBaseTestHelper.Test_SnapshotDiff_And_VerifyResult_Clear_Reset_Fields;
var
  D: TSnapshotDiff;
...
initialization
  TDUnitX.RegisterTestFixture(TTestUniBaseTestHelper);
