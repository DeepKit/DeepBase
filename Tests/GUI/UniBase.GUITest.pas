...
begin
  ...
  try
    ...
    HTML.Add('<title>UniBase GUI 测试报告</title>');
    HTML.Add('<style>');
    ...
    HTML.Add('<h1>🧪 UniBase GUI 测试报告</h1>');
...
var
  C: TControl;
begin
  C := TUniBaseTestHelper.FindControl(FForm, ControlName);
  if Assigned(C) then
    Click(C)
  else
...
begin
  ...
  TUniBaseTestHelper.SimulateClick(AControl);
...
begin
  ...
  TUniBaseTestHelper.SimulateDoubleClick(AControl);
  Wait(FConfig.StepDelay);
end;
...
begin
  ...
  TUniBaseTestHelper.SimulateInput(AControl, Text);
  Wait(FConfig.StepDelay);
end;
...
begin
  ...
  begin
    ...
    TUniBaseTestHelper.SimulateSelect(C, Index);
    Wait(FConfig.StepDelay);
  end
  else
    raise ETestAssertionFailed.CreateFmt('Control "%s" not found', [ControlName]);
...
begin
  ...
  begin
    ...
    TUniBaseTestHelper.SimulateSelect(C, Text);
    Wait(FConfig.StepDelay);
  end
  else
    raise ETestAssertionFailed.CreateFmt('Control "%s" not found', [ControlName]);
...
begin
  ...
  begin
    ...
    TUniBaseTestHelper.SimulateCheck(C, Checked);
    Wait(FConfig.StepDelay);
  end
  else
    raise ETestAssertionFailed.CreateFmt('Control "%s" not found', [ControlName]);
...
begin
  ...
  if Assigned(FForm) and (FForm.ActiveControl <> nil) then
    TUniBaseTestHelper.SimulateKeyPress(FForm.ActiveControl, Key, Shift);
...
begin
  ...
  TUniBaseTestHelper.AssertText(C, Expected, Msg);
end;
...
begin
  ...
  TUniBaseTestHelper.AssertValue(C, Expected, Msg);
end;
...
begin
  ...
  TUniBaseTestHelper.SaveScreenshotToFile(FForm, Result);
...
begin
  ...
  TUniBaseTestHelper.SaveScreenshotToFile(FForm, FileName);
end;
