unit Test.UniBase.Performance;

{*******************************************************************************
  UniBase Performance Benchmark Suite
...
begin
  FDBPath := TPath.Combine(TPath.GetTempPath, 'unibase_perf_config_' + TGUID.NewGuid.ToString + '.db');
  FManager := TUniBaseManager.Create(nil);
...
begin
  FDBPath := TPath.Combine(TPath.GetTempPath, 'unibase_perf_log_' + TGUID.NewGuid.ToString + '.db');
  FManager := TUniBaseManager.Create(nil);
  FManager.InitializeWithDB(FDBPath);
  FBenchmark := TBenchmarkRunner.Create;
end;
...
begin
  ...
  Assert.IsTrue(R.OpsPerSecond > 500000,
    Format('Cache hit should exceed 500K ops/sec, got %.0f', [R.OpsPerSecond]));
end;
...
begin
  ...
  Assert.IsTrue(EffectiveOps > 200000,
    Format('Concurrent cache ops should exceed 200K effective ops/sec, got %.0f', [EffectiveOps]));
end;
...
begin
  FDBPath := TPath.Combine(TPath.GetTempPath, 'unibase_perf_doqry_' + TGUID.NewGuid.ToString + '.db');
