...
type
  ...
  public
    ...
    [Test]
    procedure Test_MainThreadWaitMode_FluentConfig;
    [Test]
    procedure Test_OnMainThreadWait_Fires;
    [Test]
    procedure Test_MainThreadWait_Raise_PreventsBlocking;
    [Test]
    procedure Test_ExecuteAsync_RunsRetryOffMainThread;
    [Test]
    procedure Test_ExecuteAsync_Func_ReturnsValue;
...
procedure TRetryPolicyTests.Test_MainThreadWaitMode_FluentConfig;
var
  Result: TRetryPolicy;
begin
  Result := FRetryPolicy.MainThreadWaitMode(rmwRaise);
  Assert.AreSame(FRetryPolicy, Result);
end;

procedure TRetryPolicyTests.Test_OnMainThreadWait_Fires;
var
  Fired: Boolean;
  SeenRetry: Integer;
  SeenDelay: Int64;
  SeenMode: TRetryMainThreadWaitMode;
begin
  Fired := False;
  SeenRetry := 0;
  SeenDelay := 0;
  ...
  FRetryPolicy
    .MaxRetries(1)
    .FixedDelay(1)
    .OnMainThreadWaitEvent(
      procedure(Retry: Integer; DelayMs: Int64;
        var Mode: TRetryMainThreadWaitMode)
      begin
        Fired := True;
        SeenRetry := Retry;
        SeenDelay := DelayMs;
  ...
  Assert.IsTrue(Fired, 'Main-thread retry wait callback should fire');
  Assert.AreEqual(1, SeenRetry);
  Assert.AreEqual(Int64(1), SeenDelay);
  Assert.AreEqual(Integer(rmwWarn), Integer(SeenMode));
end;

procedure TRetryPolicyTests.Test_MainThreadWait_Raise_PreventsBlocking;
var
  AttemptCount: Integer;
...
begin
  ...
  Assert.WillRaise(
    ...
    end, ERetryMainThreadWaitException);
  Stopwatch.Stop;

  Assert.AreEqual(1, AttemptCount,
    'Raise mode should stop before the blocking wait and retry attempt');
  Assert.IsTrue(Stopwatch.ElapsedMilliseconds < 150,
    'Raise mode should not sleep for the configured retry delay');
end;

procedure TRetryPolicyTests.Test_ExecuteAsync_RunsRetryOffMainThread;
var
  Task: ITask;
  AttemptCount: Integer;
  WorkerThreadId: TThreadID;
  MainThreadCallbackFired: Boolean;
begin
  AttemptCount := 0;
  WorkerThreadId := MainThreadID;
  MainThreadCallbackFired := False;
  ...
  FRetryPolicy
    ...
    .OnMainThreadWaitEvent(
      procedure(Retry: Integer; DelayMs: Int64;
        var Mode: TRetryMainThreadWaitMode)
      begin
        MainThreadCallbackFired := True;
      end);

  Task := FRetryPolicy.ExecuteAsync(
    procedure
    begin
      WorkerThreadId := TThread.CurrentThread.ThreadID;
      Inc(AttemptCount);
      if AttemptCount = 1 then
        raise ETestException.Create('Transient');
    end);
  Assert.IsTrue(Task.Wait(5000), 'Async retry task should complete');

  Assert.AreEqual(2, AttemptCount);
  Assert.AreNotEqual(MainThreadID, WorkerThreadId,
    'ExecuteAsync should run retry waits outside the main thread');
  Assert.IsFalse(MainThreadCallbackFired,
    'Async execution should not trigger main-thread wait warning');
end;

procedure TRetryPolicyTests.Test_ExecuteAsync_Func_ReturnsValue;
var
...
begin
  ...
  Future := FRetryPolicy.ExecuteAsync<string>(
    function: string
    begin
      Inc(AttemptCount);
      if AttemptCount = 1 then
  ...
  Assert.AreEqual('ok', Future.Value);
  Assert.AreEqual(2, AttemptCount);
end;
