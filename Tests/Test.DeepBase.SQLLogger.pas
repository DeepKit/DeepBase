{ ============================================================================ 
  Test.DeepBase.SQLLogger - Unit tests for DeepBase.SQLLogger
...
begin
  ...
  TempFile := TPath.Combine(TPath.GetTempPath, 'DeepBase_sqllogger_test.log');
  ...
  try
    ...
    Assert.IsTrue(ContainsText(Lines[0], 'DeepBase SQL Log Export'));
  finally
    Lines.Free;
    TFile.Delete(TempFile);
  end;
...
begin
  ...
  Logs := TSQLLogger.GetRecentLogs(10);
  TempFile := TPath.Combine(TPath.GetTempPath, 'DeepBase_sqllogger_sessions.log');
  if TFile.Exists(TempFile) then
    TFile.Delete(TempFile);
