{ ============================================================================
  ...
  - TDeepBasePluginBase lifecycle
  - TPluginContext functionality
  - TDeepBasePluginManager operations
  ============================================================================ }

unit Test.DeepBase.Plugin;
...
type
  ...
  TTestPlugin = class(TDeepBasePluginBase)
  private
  ...
  private
    FManager: TDeepBasePluginManager;
    FTempDir: string;
...
begin
  ...
  Assert.AreEqual('0.3', Info.MinDeepBaseVersion);
end;
...
begin
  ...
  Assert.AreEqual('NewValue', FConfigStore.Values['Plugin.NewKey']);
end;
...
begin
  FTempDir := TPath.Combine(TPath.GetTempPath, 'DeepBasePluginTest_' + 
    FormatDateTime('yyyymmddhhnnss', Now));
  TDirectory.CreateDirectory(FTempDir);
  
  FManager := TDeepBasePluginManager.Create(FTempDir, nil);
...
var
  NewDir: string;
  Manager2: TDeepBasePluginManager;
begin
  NewDir := TPath.Combine(FTempDir, 'NewPlugins');
  Assert.IsFalse(TDirectory.Exists(NewDir));
  
  Manager2 := TDeepBasePluginManager.Create(NewDir, nil);
  try
