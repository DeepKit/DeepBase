{ ============================================================================
  ...
  修复方案: 添加插件路径验证 (IsValidPluginPath) 和数字签名验证机制 
...
begin
  inherited;
  // 创建临时插件目录
  FTempPluginsDir := TPath.Combine(TPath.GetTempPath, 'UniBasePluginTest_' + IntToStr(TThread.GetTickCount));
...
var
  PluginManager: TUniBasePluginManager;
  MaliciousPath: string;
  LoadResult: Boolean;
begin
  LogTestStart('Test_PathTraversal_WithDotDot_ShouldBeBlocked');
  
  PluginManager := TUniBasePluginManager.Create(FTempPluginsDir, nil);
  try
    FErrorFired := False;
    FLastErrorMessage := '';
    ...
    Assert.IsFalse(LoadResult, '路径遍历攻击应该被阻止，LoadPlugin 应返回 False');
...
begin
  ...
  Context := TPluginContext.Create(
    ...
    begin
      // 这个不应该被调用，因为应该在 SetConfig 中抛出异常
    end,
    nil,
    nil,
    FTempPluginsDir
  ...
  try
    ...
    Assert.IsTrue(ExceptionRaised, '设置非 Plugin. 前缀的配置应该抛出异常');
    ...
    Assert.IsFalse(ExceptionRaised, '设置合法的 Plugin. 配置应该成功');
