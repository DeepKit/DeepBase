...
async function saveTenant() {
  ...
  if (!id && !Validators.tenantName(data.name)) {
    Toast.warning('租户名称只能包含字母、数字和下划线');
    return;
  }
...
async function deleteTenant(tenant) {
  Modal.confirm({
    title: '删除租户',
    message: `确定要删除租户 "${tenant.displayName}" 吗？此操作不可恢复。`,
    confirmText: '删除',
    onConfirm: async () => {
      try {
...
async function suspendTenant(tenant) {
  Modal.confirm({
    title: '暂停租户',
    message: `确定要暂停租户 "${tenant.displayName}" 吗？暂停后该租户将无法执行新流程。`,
    confirmText: '暂停',
    onConfirm: async () => {
      try {
...
function bindEvents() {
  ...
  eventBus.on('workflow:view', (workflow) => {
    Toast.info(`查看工作流 ${workflow.id} (功能开发中)`);
  });
