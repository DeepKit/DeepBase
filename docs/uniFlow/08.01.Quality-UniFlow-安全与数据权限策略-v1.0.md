...
> **版本**: 1.0  
> **更新日期**: 2025-12-03  
> **关联文档**: `03.01.Arch-UniFlow架构设计-v1.0.md` / `04.01.Data-UniFlow数据模型-v1.0.md` / `03.02.Arch-UniFlow-版本与实验策略-v1.0.md` / `05.02.API-UniFlow-MCP支持规划-v1.0.md`
---
## 1. 文档目的
本文件定义 UniFlow 在**数据、安全与权限**方面的总体策略，用于约束：
- 不同数据的敏感级别与访问规则
- 11 角色与人类用户之间的权限边界
- 对外调用（LLM / MCP / 云同步）的数据出站规则
- 日志与审计中允许/禁止出现的内容
目标是：
> 在保持 UniFlow 强大能力的同时，确保“最小权限、可追溯、可解释、可回滚”。
---
## 2. 适用范围与优先级
本策略适用于：
- 所有 UniFlow 相关代码（Delphi / Python）和配置
- 所有 UniFlow 相关的数据表、日志与云同步数据
...
- L1
  ...
  - 聚合统计：调用次数、错误率、平均时长
- L2
  - `sessions` / `messages` / `memories` / 决策胶片与 X 光片
...
- 系统内部角色
  ...
  - UniBase 服务组件（EventBus / DB / Logging / Scheduler 等）
- 外部系统
  - Python Skill 服务
  - MCP 服务 / 第三方 API
### 4.2 RBAC + ABAC 混合模型
...
```
所有对外部接口、DB、日志、Skill 调用的入口，都必须携带或构造 `TSecurityContext`：
...
1. **用户会话主体（User Session Principal）**
   - 标识：`user:<user_id>`
   - 场景：前端点击触发、用户 Access Token 直接调用 UniFlow API。
...
2. **项目运行时服务账号（Project Runtime Service Account）**
   ...
   - 权限：
     - 仅限本项目/空间下授权资源；
     - 默认最高可访问至 L2 数据，不得直接访问 L3 机密数据；
     - 是否允许出站到外部 LLM/MCP 由军需官在项目配置中显式授权。
...
- 每个 Task 逻辑上都应具备：
  ...
  - `run_as`：本次执行采用的运行主体。
- 决策顺序：
  1. 若 Workflow/Task 定义中显式配置了 `run_as`（如 `user` / `project_runtime` / 指定服务账号），按配置执行，非法配置应在调度阶段直接失败；
  2. 若无显式配置，且存在用户会话与项目上下文，则 `run_as = initiator_principal`；
  3. 若无用户会话但存在项目上下文，则 `run_as = sa:project:<owner_project_id>:runtime`；
  4. 若为平台维护任务（无项目上下文且被标注为平台任务），则 `run_as = sa:platform:runtime`；
...
- 完全信任区（L0/L1 读写，L2 受限，L3 极少）
  ...
  - Inspector（督察）
    - 可查看系统状态与部分运行轨迹
    - 默认只查看匿名化/汇总信息，对 L2 内容需要额外授权
  - Logistics（后勤）
    - 负责读写记忆，但应通过统一 Memory 接口访问，避免随意 SQL 查询
  - Chronicler（记录员）
    - 写入审计与系统日志，但必须遵守“日志脱敏规则”
- 有限信任区（L1/L2 受控，L3 禁止）
  - Commander / Dispatcher
    - 决定任务分解与流程编排，需要访问部分 L2 数据
    - 不可直接读写 L3 密钥/凭证
  - Guard
    - 可读取必要字段进行校验，但输出/日志必须脱敏
  - Quartermaster（军需官）
    - 管理 Skill registry、配额与资源使用
    - 可访问 Skill/LLM/MCP 的“引用信息”，但具体密钥由 SecretManager 管理
  - SignalOfficer（通信官）
    - 负责外部通信，不应持久化任何 L2/L3 数据
- 不信任区（输出必须被 Guard 校验）
  - Advisor（智囊）
    - LLM 调用封装，默认不信任其输出，必须经 Guard 校验后才能进入记忆/界面
  - Executor（执行者）
...
## 6. 密钥与凭证管理
### 6.1 Secret 管理抽象
- 在 UniBase 中引入统一抽象：`UniBase.Security.Secrets`
  - 提供按 Key 读取/刷新密钥的接口
  - 实现可以先基于本地加密存储，后续可替换为系统密钥环或云 KMS
- Delphi / Python 代码**只通过 Secrets 抽象获取敏感值**，禁止：
  - 将 Key/Token 硬编码在源码中
  - 将明文 Key/Token 写入日志、错误信息或 UI
### 6.2 环境变量与配置
- 允许通过环境变量向 Python Skill / MCP 客户端注入 Key/Token，但：
...
- 设计期预留 Key 轮换机制：
  - 支持同时存在新旧两套密钥
  - 调用路径能够区分使用哪一套
  - 日志中可追踪“当时使用的是哪个 Key Profile”，而非明文
---
## 7. 日志与审计策略
### 7.1 日志内容约束
...
### 7.3 日志保留与清理
- 参考 `retention_policies` 设计：
  - 系统日志：默认保留 30~90 天，可由租户策略调整
  - 审计日志：根据合规要求保留更久（如 1~3 年）
- 定期运行清理任务：
  - 按策略删除或归档旧日志
  - 确保不会无限制增长
---
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1️⃣ 申请      2️⃣ 安全评审     3️⃣ 技术验证     4️⃣ 注册上线     │
│  ┌───────┐    ┌───────┐    ┌───────┐    ┌───────┐        │
│  │ 填写   │───▶│ 安全   │───▶│ 沙箱   │───▶│ 审批   │        │
│  │ 评估表 │    │ 仿安   │    │ 测试   │    │ 上线   │        │
│  └───────┘    └───────┘    └───────┘    └───────┘        │
...
| 风险等级 | 服务类型 | 示例 | 准入要求 |
|----------|----------|------|----------|
| **低风险** | 只读查询类 | 天气、汇率、公开数据 API | 基础评估 + 沙箱测试 |
...
| 检查项 | 说明 | 通过标准 |
|----------|------|----------|
| 认证方式 | OAuth/API Key/无 | 支持安全认证 |
| 数据加密 | 传输加密 | TLS 1.2+ |
| 数据出站 | 发送什么数据 | 符合数据分级策略 |
| 数据入站 | 接收什么数据 | 需经 Guard 校验 |
| 权限范围 | 可执行操作 | 最小权限原则 |
| 审计日志 | 是否记录调用 | 必须记录 |
| 服务可用性 | SLA 承诺 | 明确档期 |
| 故障处理 | 异常响应 | 有降级方案 |
...
| 无限制查询 | 无分页/无限制的数据查询 | 资源风险 |
| 敏感数据导出 | 批量导出 PII/机密数据 | 数据泄露风险 |
| 不可审计服务 | 不提供日志/无法追溯 | 合规风险 |
