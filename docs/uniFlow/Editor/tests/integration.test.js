...
describe('Integration - 集成测试', () => {
  ...
  describe('画布与属性面板联动', () => {
    ...
    it('属性修改应反映到节点', () => {
      ...
      properties.updateProperty('name', '新开始节点');
      
      assert.equal(node.properties.name, '新开始节点');
    });
  });
  ...
  describe('边界情况', () => {
    ...
    it('删除不存在的连接不应出错', () => {
      assert.doesNotThrow(() => {
        canvas.removeConnection('nonexistent_id');
      });
    });
  });
  ...
  describe('性能基础检查', () => {
    ...
    it('应能处理 100 个节点', () => {
      ...
      assert.isBelow(duration, 1000, '创建 100 个节点应在 1 秒内完成');
    });
    ...
    it('应能处理大量连接', () => {
      ...
      assert.isBelow(duration, 500, '创建 49 条连接应在 500ms 内完成');
    });
    ...
    it('序列化大工作流应快速', () => {
      ...
      assert.isBelow(duration, 100, '序列化应在 100ms 内完成');
    });
  });
