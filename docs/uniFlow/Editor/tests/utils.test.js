...
describe('Utils - 工具函数测试', () => {
  ...
  it('generateId() 返回的 ID 应包含时间戳和随机部分', () => {
  ...
  it('createSVGElement() 应创建 SVG 元素', () => {
  ...
  it('createConnectionPath() 路径应从起点到终点', () => {
  ...
  describe('UndoManager', () => {
    ...
    it('应限制历史记录大小', () => {
      ...
      // maxHistory is 5, so only last 5 states should exist
      let count = 0;
      ...
      assert.isAtMost(count, 4); // Can undo at most (maxHistory - 1) times
    });
    ...
    it('canUndo() 和 canRedo() 边界条件', () => {
      assert.isFalse(manager.canUndo(), 'Empty history');
      assert.isFalse(manager.canRedo(), 'Empty history');
