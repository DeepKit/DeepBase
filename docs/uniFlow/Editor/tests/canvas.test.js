...
describe('WorkflowCanvas - 画布控制器测试', () => {
  let canvas;
  let container;
  ...
  describe('节点管理', () => {
    ...
    it('addNode() 应接受自定义属性', () => {
      const props = { id: 'custom_id', name: '自定义节点' };
      const node = canvas.addNode('start', 0, 0, props);
      
      assert.equal(node.properties.id, 'custom_id');
      assert.equal(node.properties.name, '自定义节点');
    });

    it('addNode() 对于未知类型应返回 null', () => {
      const node = canvas.addNode('unknown_type', 0, 0);
      
      assert.isNull(node);
    });

    it('addNode() 应触发 nodeAdded 事件', () => {
    ...
    it('removeNode() 应触发 nodeRemoved 事件', () => {
      let eventFired = false;
  ...
  describe('连接管理', () => {
    ...
    it('addConnection() 对于不存在的节点应返回 null', () => {
      const conn = canvas.addConnection(node1.id, 'nonexistent');
      
      assert.isNull(conn);
    });

    it('addConnection() 应触发 connectionAdded 事件', () => {
      let eventFired = false;
    ...
    it('removeConnection() 应移除连接', () => {
      const conn = canvas.addConnection(node1.id, node2.id);
      canvas.removeConnection(conn.id);
      
      assert.equal(canvas.connections.length, 0);
    });

    it('removeConnection() 应触发 connectionRemoved 事件', () => {
      let eventFired = false;
  ...
  describe('选择功能', () => {
    ...
    it('selectNode() 应触发 selectionChanged 事件', () => {
      let eventData = null;
  ...
  describe('序列化', () => {
    ...
    it('toWorkflowDefinition() 应设置 next_step 关系', () => {
      const start = canvas.addNode('start', 0, 0);
