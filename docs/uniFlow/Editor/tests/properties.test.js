...
describe('PropertiesPanel - 属性面板测试', () => {
  let panel;
  let container;
  ...
  it('showEmpty() 应显示空状态提示', () => {
    panel.showEmpty();
    
    assert.include(panel.contentEl.innerHTML, '选择节点');
    assert.equal(panel.titleEl.textContent, '属性');
  });
  ...
  describe('showNode()', () => {
    ...
    it('应设置当前节点', () => {
      const node = {
        id: 'test_node',
        type: 'start',
        properties: { id: 'test_node', name: '开始' }
      };
  ...
  describe('updateProperty()', () => {
    ...
    it('应更新节点属性', () => {
      const node = {
        id: 'test',
        type: 'start',
        properties: { id: 'test', name: '旧名称' }
      };
      
      panel.showNode(node);
      panel.updateProperty('name', '新名称');
      
      assert.equal(node.properties.name, '新名称');
    });
  ...
  describe('refresh()', () => {
    ...
    it('应重新渲染当前节点', () => {
      const node = {
        id: 'test',
        type: 'start',
        properties: { id: 'test', name: '原名称' }
      };
      
      panel.showNode(node);
      node.properties.name = '新名称';
      panel.refresh();
      
      const nameInput = Array.from(panel.contentEl.querySelectorAll('input[type="text"]'))
        .find(input => input.value === '新名称');
  ...
  describe('createPropertyGroup()', () => {
    ...
    it('应创建带标签的属性组', () => {
      ...
      assert.include(group.innerHTML, '测试属性');
    });
    ...
    it('有描述的属性应显示提示', () => {
      ...
      assert.include(group.innerHTML, '这是属性描述');
