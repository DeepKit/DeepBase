...
const TestRunner = {
    ...
    container.innerHTML = this.suites.map((suite, idx) => `
      <div class="test-suite" data-suite="${idx}">
        <div class="suite-header" onclick="TestRunner.toggleSuite(${idx})">
          ...
          <span class="suite-stats">${suite.tests.length} 个测试</span>
        </div>
  ...
  updateSuiteUI(suite) {
    ...
    el.querySelector('.suite-stats').textContent = `${passed}/${total} 通过`;
    el.querySelector('.suite-icon').textContent = failed > 0 ? '❌' : '✅';
  },
