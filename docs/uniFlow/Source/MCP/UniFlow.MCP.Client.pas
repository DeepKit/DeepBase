...
begin
  ...
  if FConfig.ServerUrl.IsEmpty then
    raise Exception.Create('Server URL not configured');
...
begin
  ...
  begin
    ...
    raise Exception.CreateFmt('MCP Error %d: %s', [LErrorCode, LErrorMsg]);
  end;
  
  if not AResponse.TryGetValue<TJSONValue>('result', Result) then
    raise Exception.Create('Invalid response: missing result');
end;
