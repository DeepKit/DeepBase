...
type
  ...
  TServiceLifetime = (
    slTransient,   // 每次请求创建新实例
    slSingleton,   // 全局单例
    slScoped       // 作用域单例 (每个 Scope 一个实例)
  );
...
begin
  ...
  if RttiType = nil then
    raise Exception.CreateFmt('Cannot resolve type: %s', [string(ADescriptor.ImplementationType.Name)]);
...
begin
  ...
  try
    ...
    case Desc.Lifetime of
      ...
      slScoped:
        Result := CreateInstance(Desc);  // Scope 由 TServiceScope 管理
    end;
  finally
    FLock.Leave;
  end;
...
begin
  ...
  if Result = nil then
    raise Exception.CreateFmt('Service not registered: %s', [string(AServiceType.Name)]);
end;
