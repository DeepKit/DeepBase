unit UniFlow.Workflow.Definition;
{
  ...
  参考: 05.03.API-UniFlow-Workflow定义规范-v1.0.md
}
...
type
  ...
  private
    ...
    FLeftExpr: string;           // 左操作数表达式 (如 "{{ vars.status }}")
    FRightExpr: string;          // 右操作数表达式
  ...
  public
    ...
    /// <summary>从简单表达式解析 (如 "vars.count > 10")</summary>
  ...
  TOutputConfig = class
  private
    FVariable: string;           // 存储到的变量名
    FTransform: string;          // 转换表达式
  ...
  private
    FId: string;
    FWhenValue: TJSONValue;      // 匹配值
    FMatchExpr: string;          // 匹配表达式（如 ">= 0.9"）
    FIsDefault: Boolean;
  ...
  private
    FMode: TLoopMode;
    FCollection: string;         // 集合表达式 (forEach)
    FItemVariable: string;       // 当前项变量名
    FIndexVariable: string;      // 索引变量名
  ...
  private
    ...
    FCron: string;               // Cron 表达式
    FEventName: string;          // 事件名
