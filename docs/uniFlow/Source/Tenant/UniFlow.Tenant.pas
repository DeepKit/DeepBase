...
begin
  ...
  if GetTenantByName(AName) <> nil then
    raise Exception.CreateFmt('Tenant with name "%s" already exists', [AName]);
...
begin
  ...
  try
    ...
    begin
      Tenant := GetTenant(ATenantId);
      if Tenant = nil then
        raise Exception.CreateFmt('Tenant not found: %s', [ATenantId]);

      if not Tenant.IsActive then
        raise Exception.CreateFmt('Tenant is not active: %s', [ATenantId]);
