...
begin
  ...
  try
    ...
    begin
      ...
      raise Exception.CreateFmt('连接失败: %d %s', [LResponse.StatusCode, LResponse.StatusText]);
  except
    on E: Exception do
    begin
...
begin
  if not IsConnected then
    raise Exception.Create('未连接到 RabbitMQ');
...
begin
  ...
  begin
    ...
    raise Exception.CreateFmt('API 错误: %d %s', [LResponse.StatusCode, LResponse.StatusText]);
end;
...
begin
  ...
  raise Exception.Create('Management API 不支持事务，请使用 AMQP 客户端');
end;

procedure TRabbitMQChannel.TxCommit;
begin
  raise Exception.Create('Management API 不支持事务');
end;
