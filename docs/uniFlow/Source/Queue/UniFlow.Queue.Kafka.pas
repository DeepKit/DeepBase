    ...
    try
      ...
      try
        ...
        if LResponse.StatusCode >= 400 then
          raise Exception.CreateFmt('Kafka 发送失败: %d %s', 
  ...
  try
    ...
    try
      ...
      begin
        ...
        raise Exception.CreateFmt('创建 Kafka 消费者失败: %d', [LResponse.StatusCode]);
    finally
      LStream.Free;
    end;
  finally
  ...
  try
    ...
    try
      ...
      try
        ...
        if LResponse.StatusCode >= 400 then
          raise Exception.CreateFmt('订阅失败: %d', [LResponse.StatusCode]);
      finally
        LStream.Free;
      end;
    finally
...
begin
  ...
  if LResponse.StatusCode >= 400 then
    raise Exception.CreateFmt('提交偏移量失败: %d', [LResponse.StatusCode]);
end;
...
begin
  ...
  try
    ...
    try
      ...
      if LResponse.StatusCode >= 400 then
        raise Exception.CreateFmt('创建主题失败: %d %s',
          [LResponse.StatusCode, LResponse.StatusText]);
    finally
      LStream.Free;
    end;
...
begin
  ...
  if LResponse.StatusCode >= 400 then
    raise Exception.CreateFmt('删除主题失败: %d', [LResponse.StatusCode]);
end;
