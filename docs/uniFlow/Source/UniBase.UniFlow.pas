...
begin
  if GEngine = nil then
    raise Exception.Create('UniFlow engine not initialized. Call InitializeUniFlow first.');
  Result := GEngine;
end;
...
begin
  if not FInitialized then
    raise Exception.Create('UniFlow engine not initialized. Call Initialize first.');
end;
...
begin
  if not TFile.Exists(AFilePath) then
    raise Exception.CreateFmt('Workflow file not found: %s', [AFilePath]);
...
begin
  ...
  if JSONObj = nil then
    raise Exception.Create('Invalid workflow JSON');
...
begin
  ...
  if Workflow = nil then
    raise Exception.CreateFmt('Workflow not found: %s', [AWorkflowId]);
