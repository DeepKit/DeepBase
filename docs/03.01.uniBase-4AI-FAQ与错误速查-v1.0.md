...
UniBase 当前默认使用 Windows Credential Manager 存储 LLM API Key，SQLite 中只保存 `credman:<target>` 引用：

| 方式 | 说明 | 优点 | 缺点 |
|-----|------|------|------|
| Credential Manager | Windows 凭据管理器 | 数据库不保存真实 key，系统级管理 | 绑定当前 Windows 用户/机器 |
| 环境变量/外部 KMS | 外部密钥源 | 适合团队和生产环境 | 需要额外运维配置 |

### 配置方式

在 `LLMApiKeys` 表中：
- `IsEncrypted`: 是否加密 (1=是, 0=否)
- `EncryptionMethod`: 存储方式，默认 `CREDMAN`
- `ApiKey`: Credential Manager 引用，例如 `credman:UniBase_LLM_Default_ApiKey`

### 建议

1. **本地开发**: 使用 Credential Manager
...
- [07.01.uniBase-4AI-集成检查清单-v1.0.md](07.01.uniBase-4AI-集成检查清单-v1.0.md)
