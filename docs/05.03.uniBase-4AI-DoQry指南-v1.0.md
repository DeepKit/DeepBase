...
| 函数 | 说明 | 返回值 |
|------|------|--------|
| `UniDbSelect` | 执行 SELECT，结果写入 TFDMemTable | 行数 |
...
```
以 SQL 关键字（`SELECT`、`INSERT`、`UPDATE`、`DELETE`、`WITH`）开头的字符串将直接执行。
...
- `UniBase.DB.DoQry.pas` - 模块源码（位于 `Persistence/` 目录）
- `Tests/Test.UniBase.DB.DoQry.pas` - 单元测试
- `docs/05.01.uniBase-4AI-API参考-v1.0.md` - API 参考
