...
UniBase 提供了一套完整的基础设施数据库，包含 **24 张基础表**（含 About/打赏图像表），分为三个层级：

- **Tier 0 - 核心必需 (5张)**: 框架运行必需的表
- **Tier 1 - 推荐使用 (7张)**: 常用功能支撑表
- **Tier 2 - 扩展功能 (12张)**: LLM、异常报告等高级功能

> 说明：`sql/llm_prompts_init.sql` 还提供一组 **可选 Prompt Studio 扩展表**（5 张），用于 `TLLMManager` 的版本化提示词管理；不计入上面的 24 张基础表。
...
以下表来自 `sql/llm_prompts_init.sql`，用于 `TLLMManager` 的分层提示词能力：
...
**安全说明**: API 密钥默认写入 Windows Credential Manager；数据库只保存 `credman:` 引用，不保存明文 key。旧版明文字段可用 `Scripts/migrate_llm_credentials.ps1` 迁移。
...
#### 24. aboutMeImages - About/打赏与公众号图像（应用 DB1 专用）

用于在各应用的配置数据库（DB1，例如 `TwoKeyRunConfig.db`、`MoveCConfig.db`）中存储 About 页面、打赏二维码和公司公众号二维码等图像及其附加文字信息。

> 说明:
> - 本表主要服务于 `UniBase.VCL.AboutFrame` + AntiTamper + SeedTool 方案。
> - 推荐只在应用侧 DB1 中使用，不强制出现在 UniBase 自身的样例 Config.db 中。
...
| Enabled     | INTEGER  | 1      | 是否在 AboutFrame 中显示该项：1=显示，0=隐藏 |
| Sha256Hash  | TEXT     | -      | 明文图像的 SHA-256 哈希值，用于完整性校验 |
| HmacSha256  | TEXT     | -      | 明文图像的 HMAC-SHA256 签名（基于密钥派生），用于防篡改校验 |
| CreatedAt   | TEXT     | now()  | 创建时间 |
| UpdatedAt   | TEXT     | now()  | 更新时间 |
| Extra       | TEXT     | -      | JSON 扩展字段 |
| Remarks     | TEXT     | -      | 备注 |

> 推荐预置的 ImageKey 列表（具体内容由各应用自行播种）：
...
> - 本表的数据应通过 SeedTool 播种，保证加密参数（密钥/盐/迭代次数/HMAC 开关）与主程序一致。
> - 主程序通过 `UniBase.AntiTamper.LoadSecureImage` 之类 API 读取并验证图像及地址文本。
