...
| 场景 | 推荐 API | 说明 |
|------|----------|------|
| 直接聊天/补全调用 | `TUniBaseLLM` (`Core/UniBase.LLM.pas`) | 最小依赖，适合业务直接调模型 |
| 提示词版本管理/元提示词/BoundQuery | `TLLMManager` (`Core/UniBase.LLM.Manager.pas`) | 适合需要 Prompt 工程化治理 |
| 快速门面接入（Features） | `LLM()` Facade (`Features/UniBase.LLM.Service.pas`) | 用于快速接入，内部仍落到核心 LLM 能力 |

迁移建议：
1. 新业务优先选 `TUniBaseLLM` 或 `TLLMManager`。  
2. 现有 `LLM()` 门面调用可保留，逐步迁移到显式 API（便于测试与调优）。
...
`TLLMManager` 负责提示词、版本、元提示词和 BoundQuery；底层模型配置仍由 `TUniBaseLLM` / `LLMConfig` 提供。
...
`ExecuteAsync` 会在后台任务中使用 `Params`，因此回调完成前不要释放它；如需更新 UI，应在回调内切回主线程。
...
不需要提示词版本管理时，可以直接使用 `TUniBaseLLM`：
...
begin
  ...
  try
    ...
    if LLMClient.Chat('用一句话介绍 UniBase。', Response, 'Default') then
      ShowMessage(Response.Content)
    else
...
begin
  Result := '{}'; // 按项目约定将 Params 转为 JSON。
end;

function DataSetToJson(Data: TDataSet): TJSONArray;
begin
  Result := TJSONArray.Create; // 按项目约定将 DataSet 转为 JSON。
end;
...
CREATE TABLE LLMConfig (
    ...
    ModelId          TEXT NOT NULL,         -- gpt-4o-mini/claude-3-5-sonnet 等
    BaseUrl          TEXT,
    ApiKeyRef        TEXT,                  -- Credential Manager 引用 (credman:...) 或 LLMApiKeys.Name
...
begin
  ...
  if Response.Success then
    ...
    raise ELLMException.Create('LLM 调用被限流: ' + Response.ErrorMessage)
  else
    raise ELLMException.Create(Response.ErrorMessage);
end;
```
