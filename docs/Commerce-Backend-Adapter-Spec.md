...
> 更新日期: 2026-05-05  
> 状态: COMMERCE-002A/B/C/D 已完成；支付通知验签适配继续进行  
...
业务后端 HTTP API
        |
        +-- 后端数据库: users/orders/payments/entitlements
        +-- 微信支付/支付宝等渠道 SDK
        +-- Webhook/notify 验签
```

Delphi 客户端和网页/小程序都只调用业务后端 API，不直接连接生产数据库，也不保存支付密钥。
...
| amount_minor | int64 | required，分/厘等最小货币单位 |
| currency | string | required，例如 CNY |
...
- `(provider, provider_trade_no)`，允许 `provider_trade_no` 为空时按数据库能力处理。
...
- 微信支付优先使用平台通知 `id`。
- 无稳定通知 ID 时使用 `sha256(provider + raw_payload)`。
...
Delphi 侧的路由和 JSON 字段名常量定义在 `Features/UniBase.Commerce.Backend.Contract.pas`，后续 HTTP 适配器和 contract tests 必须复用该单元，避免文档、测试和实现各写一套字符串。

所有接口必须走 HTTPS。生产环境建议要求：

- `Authorization: Bearer <app/user token>`，或服务端到服务端 API key。
- `Idempotency-Key` 用于创建订单、创建支付意图、处理通知等可重试请求。
- 响应统一返回 JSON。
...
- 幂等：同一 `(provider, provider_user_id, app_id)` 必须返回同一个 `user_id`。
- 微信体系建议优先记录 `union_id`，后续可做多端账号合并。
...
- 后端持有微信商户号、证书、私钥、API v3 key。
- 客户端只拿支付参数，不参与签名密钥管理。
- 同一未支付订单重复调用可复用同一 payment，也可以刷新支付参数，但不能创建多条互相冲突的支付流水。
...
1. 验证微信平台证书/签名、时间戳、nonce。
2. 解密通知资源。
3. 记录 `payment_notifications.raw_payload`。
4. 用 `out_trade_no` 查订单。
5. 校验订单号、金额、币种、商户号、支付状态。
6. 在事务内更新 payment/order，并按 `source_order_id` 幂等发放 entitlement。
7. 返回微信支付要求的成功响应。

后端内部可调用 Commerce 等价流程：

```text
ConfirmPayment(notification)
```

但传入 `ConfirmPayment` 前，通知必须已经验签和校验。
...
UniBase 已提供生产向 HTTP 后端转发的 `ICommerceStorage` 适配器：

- 单元：`Features/UniBase.Commerce.Backend.Http.pas`
- 类：`TCommerceHttpStorage`
- 配置：`TCommerceBackendHttpConfig.BaseUrl / BearerToken / ApiKey / TimeoutMs`
- 默认 Header：`Accept: application/json`、`Content-Type: application/json`、可选 `Authorization: Bearer <token>`、可选 `X-API-Key`
- 测试扩展点：`ICommerceBackendHttpTransport`

后端需要按下表提供接口。Delphi 侧路由和字段常量仍以 `Features/UniBase.Commerce.Backend.Contract.pas` 为准。
...
| `FindProduct` / `UpsertProduct` | `GET/PUT /commerce/products/{app_id}/{product_id}`；生产建议只允许管理端修改 |
...
UniBase 已提供生产向 HTTP 后端转发的 `ICommercePaymentGateway` 适配器：
...
- Header：复用 `Authorization` / `X-API-Key`，并用 `Idempotency-Key: <payment_id>` 防止客户端重试创建冲突
- 响应字段：`payment_id`、`out_trade_no`、`prepay_id`、`client_params_json`、`pay_url`、`qr_code_data`
...
- 支付通知幂等键：微信通知 `id` 或 `sha256(raw_payload)`
...
- 生产全链路 HTTPS。
- 商户私钥、API v3 key、Webhook secret 只放后端密钥管理。
- 支付通知必须 fail-closed：验签失败、金额不符、币种不符、订单不存在时拒绝确认。
- 客户端返回的“支付成功”只能用于 UI 提示，不能作为发放权益依据。
- 管理端修改商品价格、权益码、订单状态必须有审计日志。
- 后端日志不得输出完整密钥、身份证、手机号、邮箱明文；必要时脱敏。
...
1. 已完成：固化本契约并评审字段命名。
2. 已完成：用内存/mock 后端写 contract tests。
3. 已完成：实现 HTTP `ICommerceStorage` 适配器。
4. 已完成：实现后端微信支付 intent API 的 Delphi `ICommercePaymentGateway` 后端代理适配器。
5. 下一步：实现微信支付 notify 验签和幂等确认。
6. 跑通一个下游端到端样例。
7. 再评估 CloudBase/Firebase/Supabase 等托管后端是否需要官方适配。

---

## 9. 封板标准

- `TInMemoryCommerceStorage` 仅用于测试和开发文档，不用于生产。
- 下游项目只依赖 `Features/UniBase.Commerce.*` 的流程入口，不再引用旧认证/计费 UI 或 AiPEX/AipexBase。
- 支付确认以可信后端通知为准。
- 后端数据库有唯一约束和事务保护。
- 文档入口只指向本契约、`UniBase-Downstream-Integration.md` 和 ThirdParty 扩展指南。
