...
## 11. HB 视觉系统与组件接入 (DeepBase.VCL.HB)

下游 VCL 业务软件（如客户挽回系统、营销工具、中台工作台）推荐使用 `DeepBase.VCL.HB.*` 现代商业视觉体系。
...
begin
  // 1. 注册 10 套内置主题 (内置 RC_DATA 资源 + 嵌入降级)
  RegisterBuiltInThemes;

  // 2. 切换当前主题与密度 (支持从 DB1 Settings 读取 App.Theme / App.ThemeDensity)
  THbTheme.ApplyTheme('huanjin-gold', hdComfortable);

  // 3. 注册主题变化广播监听 (若有自定义自绘控件)
  THbTheme.AddListener(OnHbThemeChanged);
end;
```

### 11.2 可用组件速查

- **原子控制组件 (`DeepBase.VCL.HB.Controls`)**:
  - `THbButton`: 支持 4 种形态 (`bkPrimary`, `bkGhost`, `bkSoft`, `bkDanger`)、3 种尺寸与胶囊圆角 (`Pill=True`)。
  - `THbDualButton`: 免费动作轨道与 AI 点数消耗轨道一体化双轨按钮。
  - `THbChip`: 标签与筛选胶囊，支持 5 种语意 Tone、选中反色与关闭 '✕' 按钮。
  - `THbBadge`: 状态微标，支持 Pill 胶囊与 Square 圆角方标。
  - `THbAvatar`: 文本姓名自动哈希取色背景 + 在线状态高亮光环。
  - `THbProgressRing`: 环形进度圈与不确定循环旋转等待器。
  - `THbSkeleton`: 占位骨架屏与扫光动效。
  - `THbToast`: 浮层状态反馈气泡。
  - `THbSectionHeader`: 业务分组标题、计数微标与尾部外链。
- **业务复合卡片 (`DeepBase.VCL.HB.Cards`)**:
  - `THbCard`: 容器卡片，支持 Surface / Sunken / Hero 渐变 / Outline 四种背景。
  - `THbStatBig`: 核心 KPI 数字展示（如沉睡资产、挽回人数）。
  - `THbListRow`: CRM / 任务高密度联系人列表行，集成头像、微标、动作按钮。
  - `THbEmptyState`: 空状态插画引导卡片。
...
| `Examples/CommerceE2EDemo` | Commerce 端到端流程 |
| `Tools/Gallery/hbtheme_gallery.dpr` | HB 视觉组件 12 控件交互画廊 |

本地检查：

```powershell
Scripts\build_examples_win64.ps1
Scripts\build_packages_win64.ps1 -Profile VCL
Scripts\run_tests.ps1 -Type Unit -Run "Test.DeepBase.VCL.HB.Theme" -AllowFilteredCI -CI
```
