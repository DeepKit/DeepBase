...
graph TD
    ...
    Stage1 -->|创建 root.txt| Action1["写入项目根目录绝对路径"]
    Action1 -->|初始化 config.db| DBInit["创建 Tier 0 表结构"]
    DBInit -->|无 UI 修改| Risk1["风险: 低"]
    Risk1 -->|完成| Stage2
    ...
    Stage3 -->|全局异常处理| GlobalHandler["在 .dpr 中添加<br/>Application.OnException"]
    GlobalHandler -->|记录日志| UseLogger["改用 UniBase.Log()"]
    UseLogger -->|查看日志| ViewLogs["在 Studio 中<br/>查看日志查看器"]
    ViewLogs -->|完成| Stage4
    
    Stage4["阶段 4<br/>替换 MRU 控件"]
    Stage4 -->|文件菜单| ReplaceMenu["用 TMRUPopupMenu<br/>替换手工代码"]
    ReplaceMenu -->|自动管理| MRUFunc["最近文件<br/>自动显示"]
    MRUFunc -->|完成| Stage5
    
    Stage5["阶段 5<br/>配置迁移到数据库"]
    Stage5 -->|梳理现有配置| ScanConfig["扫描 Registry/INI<br/>映射到 Settings"]
    ScanConfig -->|批量导入| ImportConfig["导入工具导入<br/>或手工添加"]
    ...
    Stage6["阶段 6<br/>文本国际化"]
    Stage6 -->|替换标签| ReplaceLabelI18n["TLabel -> TI18nLabel<br/>设置 TextKey 属性"]
    ReplaceLabelI18n -->|替换按钮| ReplaceBtnI18n["TButton -> TI18nButton"]
    ReplaceBtnI18n -->|设置字符串| SetStrings["设置为英文原文"]
    SetStrings -->|Studio 扫描| ScanI18n["Studio 源码扫描<br/>采集待翻译文本"]
    ScanI18n -->|翻译编辑| TranslateI18n["编辑翻译<br/>支持 LLM 自动翻译"]
    TranslateI18n -->|完成| Stage7
    
    Stage7["阶段 7<br/>运行 Studio"]
    Stage7 -->|启动独立工具| LaunchStudio["双击 UniBase Studio"]
