# UniBase UI 组件命名与设计规范列表

本文档列出了 UniBase 框架中所有标准 UI 组件的规范名称、用途及文件对应关系。
...
## 5. LLM 与 AI (LLM Controls)

| 组件名称                 | 对应文件                   | 说明                                     |
|:-------------------- |:---------------------- |:-------------------------------------- |
| **TLLMConfigPanel**  | `TLLMConfigPanel.svg`  | 综合面板，包含 Provider 配置、API Key 管理和调用历史表格。 |
...
| **TLogListView**    | `TLogListView.svg`    | 实时日志显示组件，自动连接 Logger，支持表头分列和颜色分级。   |
...
## 7. UniBase Studio 工具界面 (Studio Interfaces)

这些是 UniBase Studio 独立管理工具的核心功能界面。
...
| **Studio_TranslationEditor** | `Studio_TranslationEditor.svg` | 专业的 i18n 翻译网格，支持多语言对照、状态标记和 AI 翻译。 |
| **Studio_LogViewer**         | `Studio_LogViewer.svg`         | 高级日志查看器，支持按级别/时间筛选、全文搜索和堆栈详情。      |
...
## 9. 认证与授权控件 (Authentication & License)
