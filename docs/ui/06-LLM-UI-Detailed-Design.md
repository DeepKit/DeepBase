...
本文档详细说�?DeepBase 框架中新增的 LLM 相关 UI 界面的设计规范、布局、交互流程和实现要点�?
...
├─────────────────────────────────────────�?
�?                                        �?
�?           [🔄 SVG 动画]               �?�?上部：SVG 动画�?(80px �?
�?                                        �?
├─────────────────────────────────────────�?
�? 正在处理您的请求...                    �?
�? 当前：访�?LiteLLM...                  �?�?中部：动态文�?(60px �?
�? Token 使用: 45/2000                    �?
├─────────────────────────────────────────�?
�?                                        �?
�?    [取消]              [后台运行]      �?�?下部：按钮区 (50px �?
...
- **高度**: 35px
- **标题文本**: "LLM 处理�?.." (14pt, 加粗)
- **关闭按钮**: 右上�?X 按钮，点击触�?"取消" 操作

#### 1.4.2 上部：SVG 动画�?(Animation Area)
- **高度**: 80px
- **内容**: 
  - �?AnimationAssets 库中随机选择一�?SVG 动画
  - 动画自动播放、循环、无限重�?
  - 尺寸不超�?60px × 60px，居中显�?
  - 动画库示例：加载圈、脉冲圆、旋转立方体�?
...
───────────────────────────────────────────────────────────
IDS_LLMWAIT_TITLE       | LLM 处理�?..       | Processing with LLM...
...
让用户配�?LLM 连接参数（如 API Key、Model、Temperature、成本等），查看历史调用记录�?

### 2.2 规格

| 属�?| �?|
|------|-----|
| **控件类型** | Panel (嵌入到主设置窗体或独�?Dialog) |
| **默认大小** | 800px × 500px (或父窗体尺寸) |
| **分为两部�?* | 上部配置面板 (60%) + 下部 Grid (40%) |

### 2.3 UI 布局

#### 2.3.1 上部：配置面�?(Configuration Panel)
...
├──────┼──────────┼───────┼────────┼──────────┼────────┼───────────�?   �?
�?4:30 │LiteLLM   │gpt-4o �?  �?   �?1250ms   �? 245   �? $0.08    �?   �?
�?4:28 │LiteLLM   │gpt-4o �?  �?   �? 980ms   �? 180   �? $0.06    �?   �?
�?4:27 │OpenAI    │gpt-4o �?  �?   �?5400ms   �? 512   �? $0.15    �?   �?
�?4:25 │LiteLLM   │gpt-3.5�?  �?   �? 2100ms  �? 100   �? $0.01    �?   �?
�?4:20 │OpenAI    │gpt-4o �?  �?   �?3800ms   �? 340   �? $0.10    �?   �?
├──────┴──────────┴───────┴────────┴──────────┴────────┴───────────�?   �?
...
- **选项示例**:
  ...
  - Custom (自定�?
- **默认�?*: "LiteLLM"
- **关联**: 更改 Provider 会自动更新可�?Model 列表
...
| 列名 | 宽度 | 说明 |
|------|------|------|
| **Time** | 100px | 调用时间�?(HH:MM:SS) |
| **Provider** | 90px | LLM 提供商名�?|
| **Model** | 80px | 使用的模�?|
| **Status** | 60px | �?成功 / �?失败 / �?进行�?|
| **Duration** | 100px | 响应时间 (ms) |
| **Tokens** | 80px | 使用�?Token �?|
| **Est. Cost** | 90px | 估算费用 ($) |

#### Grid 功能
- **排序**: 点击列标题排�?
- **筛�?*: Provider/Status 的快速筛�?
- **搜索**: 支持全文搜索
- **导出**: 导出�?CSV/Excel
- **刷新**: 重新加载最新记�?
- **上下文菜�?*: 右键菜单 (复制、删除、查看详�?
...
## 3. 后台任务通知�?(Background Task Notification Bar)

### 3.1 用�?

当用户选择"后台运行"时，在主窗体的状态栏或专用通知栏显�?LLM 任务进度�?
...
| **位置** | 主窗体状态栏下方或系统通知�?|
| **高度** | 40px (如为内部通知�? |
| **显示条件** | 存在运行中的后台 LLM 任务�?|
| **自动隐藏** | 任务完成�?5 秒自动隐藏或点击关闭 |
...
#### LLMCallHiDeepDeepDeepDeepDeepStory �?

```sql
CREATE TABLE LLMCallHiDeepDeepDeepDeepDeepStory (
...
1. **加密存储**: 所�?API Key 使用 AES-256 加密
2. **内存安全**: 使用 SecureString 避免明文在内存中泄露
3. **日志安全**: 从不在日志中记录完整�?API Key (最多显示前 4 �?
4. **传输安全**: 所�?API 请求使用 HTTPS
