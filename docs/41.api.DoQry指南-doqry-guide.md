...
## PostgreSQL 连接装配与安全凭据

### 1. 驱动装配
`DeepBase.DB.DoQry` 内部已引入 `FireDAC.Phys.PG` / `FireDAC.Phys.PGDef`，在构建连接时无需手动注册驱动。推荐使用 `TDBConnectionProfile` 或 `TUniConnectionPool` 进行连接管理：
...
var
  ...
  Ctx: TUniQueryContext;
begin
  Profile := TDBConnectionProfile.PostgreSQL('127.0.0.1', 'bcw_runtime', 'postgres', Password, 5432);
  Conn := TDBConnectionFactory.CreateConnection(Profile, True);
  Ctx := UniDbMakeContext(Conn, udbPostgreSQL);
  // 执行 DoQry 调用...
end;
```

### 2. 凭据接入规范（SSOT）
- **凭据管理**：密码等敏感信息一律走 `DeepBase.Security`（Windows 下 DPAPI 保护）或 `TCredentialManager` / `credman:` 引用。
- **环境回退**：自动化测试/容器环境下，可通过安全环境变量（如 `DB3_PASSWORD`）接入。
- **严禁事项**：严禁在代码、配置文件或数据库普通表中存储明文密码。

### 3. 方言特性与已知限制
- **自增主键返回**：PostgreSQL 路径通过 `RETURNING id`（由 `UniDbInsertReturningId` 自动追加或由 SQL 显式指定）单次往返获取生成的主键。
- **参数类型自适应**：
  - 标准 UUID 格式字符串（36 字符）自动识别并绑定为 `ftGuid`，避免 PG 报类型不匹配错误。
  - 整数数值按 `ftLargeint` (Int64) 绑定，完全支持 >2^53 范围（如雪花 ID、大整型业务主键）。
- **多语句限制**：PostgreSQL prepared protocol 不支持单条命令拼写多条 SQL，多表清理或级联操作须在 `UniDbRunInTx` 事务内分次调用 `UniDbExec`。

---

## 参考

- `DeepBase.DB.DoQry.pas` - 模块源码（位于 `Persistence/` 目录）
- `Tests/Test.DeepBase.DB.DoQry.pas` - 单元测试与 PG 真实连接用例
- `docs/40.api.API参考-api-reference.md`
