...
UniBase 生态
│
├── Core 库（被应用引用，无 UI 依赖；UniBaseCore.dpk 不直接依赖 VCL/FMX/FireDAC）
│   ├── Config        配置读写
│   ├── i18n          国际化运行时
│   ├── Logging       日志记录
│   ├── MRU           最近使用列表
...
│
├── UI 控件包（VCL + FMX 分别实现）
│   ├── VCL Theme/Exception Adapter（承接 TStyleManager / Application.OnException）
...
- `UniBaseCore.dpk` 只能承载 UI/DB 中立的运行时核心；不得直接 `requires vcl/fmx/dbrtl/FireDAC`。
- `UniBaseServices.dpk` 承载 IoC/EventBus/Scheduler/Crypto 等服务能力；`UniBaseFeatures.dpk` 依赖 Services，避免底层服务单元重复打包。
- `UniBaseVCL.dpk` / `UniBaseFMX.dpk` 承载 UI 控件与平台适配器；VCL 主题切换、VCL 全局异常展示不得回流到 Core。
- `UniBasePersistence.dpk` 承载 FireDAC/DB 适配器；`UniBaseFeatures.dpk` 承载 Commerce 流程契约与内存测试实现；真实后端服务能力不作为本轮前端/框架封板条件。
...
│   ├── UniBase.Manager.pas           # 核心管理器与 root/config.db 解析
│   ├── UniBase.Config.pas            # 配置读写
│   ├── UniBase.i18n.pas              # 国际化
│   ├── UniBase.Logging.pas           # 日志
│   ├── UniBase.MRU.pas               # 最近使用
│   ├── UniBase.Hotkeys.pas           # 快捷键
│   ├── UniBase.Theme.pas             # 主题元数据与平台适配分发
│   ├── UniBase.Exception.pas         # 异常记录与平台适配分发
│   ├── UniBase.License.pas           # License 验证
│   ├── UniBase.LLM.pas               # LLM 兼容门面（由 UniBaseFeatures.dpk 承载）
│   ├── UniBase.Security.pas          # 加密/脱敏
│   └── UniBase.Types.pas             # 公共类型定义
│
├── Persistence/                      # DB/FireDAC 持久化适配器
│   ├── UniBase.DB.DoQry.pas          # DoQry 业务 SQL 门面
│   ├── UniBase.DB.Pool.pas           # 连接池
│   ├── UniBase.DB.Factory.pas        # DB 工厂
│   └── UniBase.Persistence.*.pas     # Core 端口的 FireDAC 实现
│
├── Features/                         # 可选功能，不进入最小核心
│   ├── UniBase.LLM.*.pas             # LLM 配置、HTTP、门面服务扩展
│   ├── UniBase.Commerce.*.pas        # 用户/订单/支付/权益流程契约
│   ├── UniBase.Updater.pas           # 发布更新
│   ├── UniBase.AutoUpdate.pas        # 自动更新门面
│   ├── UniBase.AntiTamper.pas        # 防篡改
│   ├── UniBase.Unlock.pas            # 解锁
│   ├── UniBase.CloudSync.pas         # 云同步
│   ├── UniBase.CloudBackup.pas       # 云备份
│   ├── UniBase.Graph.pas             # 图结构/算法
│   ├── UniBase.Math.pas              # 数学/统计/插值工具
│   └── UniBase.HttpServer.pas        # 轻量 HTTP Server
│
├── VCL/
│   ├── UniBase.VCL.ThemeAdapter.pas  # VCL TStyleManager 适配
│   ├── UniBase.VCL.ExceptionAdapter.pas # VCL Application.OnException 适配
...
│   ├── UniBaseServices.dpk           # 服务层运行时包
│   ├── UniBasePersistence.dpk        # 持久化运行时包
│   ├── UniBaseFeatures.dpk           # 可选功能运行时包
...
└── Tools/
    ...
    └── CLI/                          # UniBase CLI 源码
        ...
        ├── UniBase.CLI.Interactive.pas # 交互式 REPL
        ├── UniBase.CLI.Pipeline.pas    # 管道与过滤器
        └── UniBase.CLI.SSH.pas         # SSH 远程执行
```
...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│  UniBase.Core   │  ← 最小核心（无 UI/DB/可选功能依赖）
└────────┬────────┘
         │
    ┌────┼──────────────┬──────────────┐
    ▼    ▼              ▼              ▼
┌────────────┐ ┌────────────────┐ ┌──────────┐
│ Services   │ │ Persistence    │ │ Features │  ← 服务、DB、Commerce/LLM 等可选能力
└────┬───────┘ └───────┬────────┘ └────┬─────┘
     └────────┬────────┴───────────────┘
              ▼
        ┌────────────┐
...
1. UniBaseCore.dpk
2. UniBaseServices.dpk
3. UniBasePersistence.dpk
4. UniBaseFeatures.dpk（依赖 UniBaseServices.dpk）
5. UniBaseVCL.dpk 或 UniBaseFMX.dpk
6. dclUniBaseVCL.dpk 或 dclUniBaseFMX.dpk（设计时包）
7. Studio、Tray、CLI 等工具
