...
- **DOQRY-003**: 新增 `docs/05.03.uniBase-4AI-DoQry指南-v1.0.md` 使用指南（319 行）
...
- 补全 `data/样例Config.db` 所有 17 个表
  ...
  - Security: Secrets
- 更新 [ARCH-QUICKSTART.md](../ARCH-QUICKSTART.md) 添加样例数据库说明
...
- **修改文件**:
  ...
  - `docs/03.05.uniBase-4H-LLM架构设计-v1.0.md` - 新增 LLM 模块架构文档
- **新增表**:
...
- **修改文件**:
  - `Persistence/UniBase.DB.ConnectionPool.pas`: 新增 `EConnectionPoolException` / `EConnectionPoolTimeout`，`Acquire` 超时时抛出特定异常
...
- **修改文件**:
  ...
  - `Features/UniBase.Updater.pas`: 更新 `VerifySignature` 方法调用 `TRSAVerifier`
...
- **修改文件**: `Tools/CLI/UniBase.CLI.SSH.pas` (v0.1 → v0.2)
...
- **修改文件**: `Features/UniBase.CloudSync.pas`
...
- **完成日期**: 2025-12-02
- **描述**: 创建独立的快速入门文档
- **输出物**: 快速入门文档（当前入口见仓库根目录 [ARCH-QUICKSTART.md](../ARCH-QUICKSTART.md)）
- **内容**:
  - 系统要求和安装步骤
  - 控制台最小示例
  - VCL 应用完整示例 (DPR + 窗体)
...
- **修改文件**: `Tools/CLI/UniBase.CLI.SSH.pas` (v0.2 → v0.3)
- **新增 API**:
...
### ENTROPY-001: 创建 Glossary 术语表 ✅
- **完成日期**: 2025-12-07
- **输出物**: `docs/02.01.uniBase-4AI-术语表-v1.0.md` (48 行)
- **内容**: 定义 UniBase/UniFlow 核心术语的规范名称和禁用同义词
...
- **完成日期**: 2025-12-07
- **说明**: UniFlow.AI.Types.pas 和 UniFlow.AI.LLMClient.pas 已删除，统一使用 UniBase.LLM
- **输出物**: UniFlow.AI.Adapter.pas 已创建作为轻量适配层
...
- **内容**:
  ...
  - 添加 `TErrorCode` record 类型，支持 `{Source}/{Category}/{Specific}` 格式解析和构建
  - 将所有错误码常量从旧格式 (`INVALID_WORKFLOW`) 改为新格式 (`UniFlow/Validation/InvalidWorkflow`)
  - 保留 v1 兼容常量 (deprecated)

### 低熵约束文档建立 ✅
- **完成日期**: 2025-12-07
- **输出物**:
  - AI 编程低熵约束主指南 - 已归并到当前规范体系
  - uniFlow 专用约束 - 已归并到当前规范体系
  - uniBase 专用约束 - 已归并到当前规范体系
  - 外部系统集成约束 - 已归并到当前规范体系
- **内容**:
  - AI 约束规则 (复用优先、Glossary 一致性、分层隔离、错误语义统一、Event 语义约束)
  - 人类指导 (Prompt 模板、Code Review 检查清单)
  - 量化指标框架
  - 良好实践与反面教材示例

### ENTROPY-004: 分层架构边界检查脚本 ✅
- **完成日期**: 2025-12-07
- **输出物**: `scripts/check-layer-violations.ps1` (362 行)
- **功能**:
  - 检测反向依赖 (核心层依赖外壳层)
  - 检测循环依赖
  - 输出 JSON 格式报告
...
## 2025-12-07 安全漏洞修复与低熵重构

### uniFlow 安全漏洞修复（方向 P: TASK-2100~2104）

| 任务 ID | 名称 | 文件 | 修复内容 |
|---------|------|------|----------|
| TASK-2100 | 沙箱逃逸防护 | `code_executor.py` | 移除 `setattr/delattr`，添加 `safe_getattr` 和 `DANGEROUS_ATTRS` 黑名单 |
| TASK-2101 | SSRF 防护 | `http-request.js` | 添加 `isBlockedUrl()` 阻止内网请求 |
| TASK-2102 | JSON 解析容错 | `node-types.js` | 添加 `safeJsonParse()` 辅助函数 |
| TASK-2103 | 错误信息脱敏 | `main.py` | 生产环境不返回详细错误堆栈 |
| TASK-2104 | 存储操作反馈 | `TenantConsole/utils.js` | `Storage.set()` 返回 boolean |

### uniFlow 代码质量增强（方向 Q: TASK-2105~2109）

| 任务 ID | 名称 | 文件 | 修复内容 |
|---------|------|------|----------|
| TASK-2105 | Analytics 存储反馈 | `Analytics/utils.js` | 修复 `setStorage()` 静默失败 |
| TASK-2106 | Timeline 日志完善 | `timeline.js` | `formatJson()` catch 块添加日志 |
| TASK-2107 | XSS 防护增强 | `dashboard.js` | `escapeHtml()` 增加反引号/斜杠转义 |
...
### UniBase Core Bug 修复（BUG-041~049）

| Bug ID | 严重度 | 文件 | 修复内容 |
|--------|--------|------|----------|
| BUG-041 | HIGH | `UniBase.Scheduler.pas` | `ExecuteTask` 使用 `FLock` 保护计数器 |
| BUG-042 | LOW | `UniBase.MRU.pas` | 支持 UNC 路径 (`\\server\share`) |
| BUG-043 | MEDIUM | `UniBase.EventBus.pas` | 泛型 Filter 包装为非泛型 |
| BUG-044 | MEDIUM | `UniBase.DB.DoQry.pas` | `UniDbSetCacheTTL` 增加空锁检查 |
| BUG-045 | MEDIUM | `UniBase.RateLimiter.pas` | `TTokenBucketLimiter.Create` 参数验证 |
| BUG-046 | MEDIUM | `UniBase.WorkerQueue.pas` | `WaitForCompletion` 同时检查 Pending/Running |
| BUG-047 | LOW | `UniBase.WorkerQueue.pas` | 使用 `TInterlocked.Add` 原子累加 |
| BUG-048 | MEDIUM | `UniBase.Authorization.pas` | 添加 `MapActionString` Action 映射 |
| BUG-049 | MEDIUM | `UniBase.HttpServer.pas` | 添加 `Bytes` 方法支持二进制响应 |

### 低熵约束任务

| 任务 ID | 名称 | 输出物 | 状态 |
|---------|------|--------|------|
| ENTROPY-004 | 分层架构边界检查 | `scripts/check-layer-violations.ps1` | ✅ 已完成 |
| ENTROPY-005 | 命名一致性审计 | 命名审计报告 | ✅ 已完成 |
| ENTROPY-007 | 消除 LLM.Providers 重复代码 | `UniFlow.LLM.Providers.pas` 重构 | ✅ 已完成 |
| ENTROPY-008 | 统一 LLM Provider 接口 | `UniBase.LLM.ILLMProvider` 接口 | ✅ 已完成 |
| ENTROPY-006 | Event Sourcing 合规检查 | `UniFlow.Workflow.State.pas` 修复 | ✅ 已完成 |
| ENTROPY-009 | 全局术语一致性修复 | 14 处错误消息术语修正 | ✅ 已完成 |
| ENTROPY-010 | 错误码格式统一 | 18 处错误码规范化 | ✅ 已完成 |
| ENTROPY-011 | 异常处理模式统一 | 9 处空 except 块添加日志 | ✅ 已完成 |
| ENTROPY-012 | 硬编码常量消除 | `UniFlow.Constants.pas` 常量文件 | ✅ 已完成 |
| ENTROPY-013 | 核心层边界明确化 | 当前技术规范与文档索引 | ✅ 已完成 |
| ENTROPY-014 | 接口契约文档化 | 当前技术规范与 API 参考 | ✅ 已完成 |
| ENTROPY-015 | 低熵 CI 检查脚本 | `scripts/check-entropy.ps1` | ✅ 已完成 |
| ENTROPY-016 | 熵度量仪表盘 | 集成到 check-entropy.ps1 | ✅ 已完成 |
| ENTROPY-017 | Python Skills 日志规范化 | 审计通过（已使用 structlog） | ✅ 已完成 |
| ENTROPY-018 | Node.js Skills 错误处理 | 审计通过（已使用 SkillError） | ✅ 已完成 |
| INSIGHT-001 | 洞见金路径 Workflow | `insight_decision_gold.json` | ✅ 已完成 |
| INSIGHT-002 | 洞见多角色 Skills | 6 个 Python Skills | ✅ 已完成 |
| INSIGHT-003 | 产品哲学校验规则 | 集成到 film_generator.py | ✅ 已完成 |
| INSIGHT-004 | 模式切换支持 | Workflow 条件步骤实现 | ✅ 已完成 |
| INSIGHT-005 | 指标与审计埋点 | structlog 日志支持 | ✅ 已完成 |
