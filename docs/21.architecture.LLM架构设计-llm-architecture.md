...
DeepBase LLM 模块提供完整�?LLM（大语言模型）集成和 Prompt 管理能力，支持：

- **�?Provider 支持**：OpenAI、Azure、Claude、本地模型等
- **Prompt 版本管理**：每个提示词最�?4 个版本，支持 A/B 测试
- **元提示词合并**：PREFIX/SUFFIX/WRAP 三种模式
- **变量模板**：`{{variable}}` 语法，支�?BoundQuery 上下文注�?
- **调用统计**：Token 用量、成本、成功率追踪

## 核心组件

```
DeepBase.LLM.pas           - LLM 客户端抽象层
DeepBase.LLM.Manager.pas   - Prompt 管理�?
DeepBase.LLM.ImportExport.pas - 导入导出工具
```
...
每个提示词最�?4 个版本，支持 A/B 测试�?

| 字段 | 类型 | 说明 |
|-----|------|------|
| PromptId | INTEGER | 所属提示词 |
| VersionNumber | INTEGER | 版本�?(1-4) |
| Content | TEXT | 提示词内�?|
| IsProduction | INTEGER | 是否为生产版�?|
...
| Priority | INTEGER | 合并优先�?|
| Level | INTEGER | 0=框架�? 1=项目�?|
...
[
  ...
  {"name": "text", "type": "string", "description": "待翻译文�?, "required": true}
]
```
...
`LLMConfiguration` 表存�?Provider 配置�?
