...
不要只 `uses DeepBase.Manager`。`DeepBaseCore` 不再依赖 FireDAC；Manager 的连接工厂由 `DeepBase.Persistence.Manager.FireDAC` 的 unit initialization 注册。源码直编时把该单元放进最终 `.dpr` 或启动 bootstrap 单元；运行时包模式要依赖并部署 `DeepBasePersistence.bpl`。`DeepBase.Persistence.RuntimeRegistration` 不是这个注册入口，它只注册 RuntimeContext 的 DBPool 组件。
...
看到这条错误，优先检查 adapter 单元是否被最终程序链接，以及是否混用了源码版 `DeepBase.Manager` 和 package 版 `DeepBasePersistence`。
...
## 13. 下游集成检查清�?
- [ ] `.dpr` 或启动 bootstrap 单元引用 `DeepBase.Persistence.Manager.FireDAC`，并且早于第一次 `DeepBase.Initialize*`。
- [ ] 运行时包模式已依赖并部署 `DeepBasePersistence.bpl`。
- [ ] 没有把 `DeepBase.Persistence.RuntimeRegistration` 当作 Manager FireDAC adapter 注册入口。
- [ ] 初始化使�?`InitializeOrRaise` �?`InitializeWithDBOrRaise`�?
- [ ] 启动失败时没有出现 `No DB connection adapter registered`；若出现，先查 adapter 是否链接、源码/package 是否混用。
- [ ] DB1/DB2/DB3/DB4 边界清楚�?
- [ ] Secret/API Key 没有明文落库或落日志�?
- [ ] UI 线程没有同步重试、同�?LLM、长时间阻塞�?
- [ ] Speech/ASR 密钥�?`DeepBase.Security`，识别请求不�?UI 线程同步执行�?
- [ ] Commerce 生产环境不使用内存存储�?
- [ ] 支付成功只信任后端通知/查单�?
- [ ] LLM �?SQLite+PostgreSQL 适配层未在下游私改�?
