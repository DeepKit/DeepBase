...
完整的实时通信应用模板，展示 UniBase ORM、EventBus 和实时消息架构。
...
```
RealtimeChatApp/
├── Chat.Types.pas      # 类型和实体定义
├── Chat.Services.pas   # 业务服务层
└── README.md
```
...
| 实体 | 说明 |
|------|------|
| `TChatUser` | 用户 (状态/头像/昵称) |
| `TChatRoom` | 聊天室 (类型/成员数) |
...
| 服务 | 功能 |
|------|------|
| `TChatService` | 消息 CRUD、反应、已读 |
| `TRoomService` | 聊天室管理、成员管理 |
| `TUserService` | 用户管理、联系人 |
| `TPresenceService` | 在线状态、正在输入 |
...
// 发送图片
var ImageMsg := ChatService.SendMediaMessage(
  RoomId,
  CurrentUserId,
...
// 设置管理员
RoomService.UpdateMemberRole(RoomId, UserId, mrAdmin);
...
// 检查在线
if PresenceService.IsOnline(UserId) then ...
...
// 获取正在输入的用户
var TypingUsers := PresenceService.GetTypingUsers(RoomId);
```
...
// 获取未读数
var UnreadCount := ChatService.GetUnreadCount(RoomId, UserId);
```
...
可以实现此接口对接 WebSocket、SignalR、Socket.IO 等实时通信协议。
...
1. **消息已读**: 添加 MessageReadReceipts 表记录每条消息的已读状态
2. **消息搜索**: 集成全文搜索 (FTS5)
