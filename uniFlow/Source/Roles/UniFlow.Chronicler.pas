unit UniFlow.Chronicler;

{*******************************************************************************
  UniFlow.Chronicler - 记录员角色 (L1 基础层)
...
type
  ...
  TChronicler = class(TUniFlowRoleBase, IChronicler)
  private
...
constructor TChronicler.Create(const AConfig: TUniFlowConfig);
var
  Meta: TRoleMetaInfo;
begin
...
var
  LogFileName: string;
begin
  LogFileName := FLogPath + PathDelim + 'uniflow_' + FormatDateTime('yyyymmdd', Now) + '.log';
...
function TChronicler.DoHandleMessage(const AMessage: TUniFlowMessage): TUniFlowMessage;
var
