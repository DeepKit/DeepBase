...
type
  ...
  /// <summary>UniFlow 配置</summary>
  TUniFlowConfig = class
  private
    FConfigPath: string;
...
function GlobalConfig: TUniFlowConfig;

implementation

var
  _GlobalConfig: TUniFlowConfig = nil;

function GlobalConfig: TUniFlowConfig;
begin
  if _GlobalConfig = nil then
    _GlobalConfig := TUniFlowConfig.Create;
  Result := _GlobalConfig;
end;

{ TUniFlowConfig }

constructor TUniFlowConfig.Create;
begin
  Create('Config\uniFlow.config.json');
end;

constructor TUniFlowConfig.Create(const AConfigPath: string);
begin
  inherited Create;
...
destructor TUniFlowConfig.Destroy;
begin
...
procedure TUniFlowConfig.ApplyDefaults;
begin
  // 基础配置
  FUseMockAI := True;
...
procedure TUniFlowConfig.LoadFromFile(const APath: string);
var
  JSONContent: string;
...
procedure TUniFlowConfig.Reload;
begin
  ApplyDefaults;
...
function TUniFlowConfig.ParseLogLevel(const ALevel: string): TLogLevel;
begin
...
function TUniFlowConfig.GetString(const APath: string; const ADefault: string): string;
begin
...
function TUniFlowConfig.GetInteger(const APath: string; const ADefault: Integer): Integer;
begin
...
function TUniFlowConfig.GetBoolean(const APath: string; const ADefault: Boolean): Boolean;
begin
...
function TUniFlowConfig.GetFloat(const APath: string; const ADefault: Double): Double;
begin
