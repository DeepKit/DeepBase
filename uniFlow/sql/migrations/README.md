...
本目录管理 UniFlow 的数据库迁移脚本，确保数据库结构的版本化和可追踪性。
...
def main():
    parser = argparse.ArgumentParser(description="UniFlow Database Migration Tool")
    ...
    parser.add_argument("--db", default="uniflow.db", help="Database path")
...
```bash
# 执行所有迁移
python migration_tool.py migrate --db ../data/uniflow.db

# 查看状态
python migration_tool.py status --db ../data/uniflow.db
```
...
```bash
# 回滚到指定版本
python migration_tool.py rollback 0003 --db ../data/uniflow.db
```
...
```bash
# 1. 备份数据库
sqlite3 uniflow.db ".backup uniflow_backup_$(date +%Y%m%d).db"

# 2. 执行迁移
python migration_tool.py migrate --db uniflow.db

# 3. 验证
python migration_tool.py status --db uniflow.db
```
...
- `04.01.Data-UniFlow数据模型-v1.0.md` - 数据模型设计
- `06.14.Dev-UniFlow-IPC通信详细设计-v1.0.md` - 消息表设计
- `03.16.Arch-UniFlow-消息总线可靠性设计-v1.0.md` - 消息持久化
