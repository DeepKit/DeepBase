#!/usr/bin/env python3
"""
UniFlow Skill 脚手架 CLI 工具
============================
快速创建符合 UniFlow 规范的 Skill 项目骨架。

使用方法：
    python uniflow-skill-create.py <skill_name> [options]

示例：
    python uniflow-skill-create.py text_summarize
    python uniflow-skill-create.py image_analyzer --type python --author "张三"
    python uniflow-skill-create.py data_export --type delphi
...
logger = logging.getLogger("uniflow.skill.{skill_name_lower}")
...
DELPHI_SKILL_TEMPLATE = '''unit UniFlow.Skill.{skill_class_name};
...
将 `UniFlow.Skill.{skill_class_name}.pas` 添加到项目中。
...
```pascal
uses UniFlow.Skill.{skill_class_name};
...
def create_skill(
    ...
    template_vars = {
        ...
        "entry_point": f"{skill_name_lower}.py" if skill_type == "python" else f"UniFlow.Skill.{skill_class_name}.pas"
    }
    ...
    if skill_type in ("delphi", "both"):
        delphi_path = base_dir / f"UniFlow.Skill.{skill_class_name}.pas"
...
def main():
    parser = argparse.ArgumentParser(
        description="UniFlow Skill 脚手架工具 - 快速创建符合规范的 Skill 项目",
...
示例:
  ...
  %(prog)s translator --type both --output ./my_skills
    ...
    parser.add_argument(
        "--author", "-a",
        default="UniFlow Team",
        help="作者 (默认: UniFlow Team)"
    )
...
║    4. 注册到 UniFlow: uniflow skill register <path>          ║
╚══════════════════════════════════════════════════════════════╝
            """)
