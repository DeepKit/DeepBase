unit UniBase.Commerce.Adapter.Supabase;

{==============================================================================
  UniBase.Commerce.Adapter.Supabase - Supabase Storage Adapter
...
class function TSupabaseConfig.Create(const ABaseUrl, AApiKey: string;
  const ATablePrefix: string; ATimeoutMs: Integer): TSupabaseConfig;
begin
...
begin
  ...
  if Response.StatusCode >= 300 then
    raise EUniBaseCommerceError.CreateFmt('Supabase GET %s: %d %s',
...
begin
  ...
  if Response.StatusCode >= 300 then
    raise EUniBaseCommerceError.CreateFmt('Supabase POST %s: %d %s',
      [AUrl, Response.StatusCode, Response.StatusText]);
...
begin
  ...
  try
    Response := FClient.Patch(AUrl, BodyStream, AddAuthHeaders);
  finally
    BodyStream.Free;
  end;
  if Response.StatusCode >= 300 then
    raise EUniBaseCommerceError.CreateFmt('Supabase PATCH %s: %d %s',
      [AUrl, Response.StatusCode, Response.StatusText]);
