(* ============================================================================
  ...
  Description:
    ...
    This module is intentionally simpler than UniBase.Updater. It is
    ...
    Version comparison:
      - Uses UniBase.Updater.TSemanticVersion to compare CurrentVersion
        with remote version ("version" field).
      - If remote <= current, CheckForUpdate returns False.
  ============================================================================ *)

unit UniBase.AutoUpdate;
...
type
  /// <summary>
  /// Update channel used by AutoUpdate. Alias of UniBase.Updater.TUpdateChannel.
  /// Only stable/beta/dev are used by version.json; alpha maps to dev.
  /// </summary>
  TUpdateChannel = UniBase.Updater.TUpdateChannel;
  TUpdateInstallMode = UniBase.Updater.TUpdateInstallMode;
...
constructor TUniBaseAutoUpdate.Create(const AUpdateUrl, ACurrentVersion: string);
begin
  inherited Create;
...
function TUniBaseAutoUpdate.GetChannel: TUpdateChannel;
begin
  Result := FChannel;
end;

procedure TUniBaseAutoUpdate.SetChannel(const Value: TUpdateChannel);
begin
  FChannel := Value;
end;

function TUniBaseAutoUpdate.GetCurrentVersion: string;
begin
...
procedure TUniBaseAutoUpdate.SetCurrentVersion(const Value: string);
begin
  FCurrentVersion := Trim(Value);
end;

class function TUniBaseAutoUpdate.ChannelKey(Channel: TUpdateChannel): string;
begin
...
class function TUniBaseAutoUpdate.NormalizeVersion(const S: string): string;
begin
  Result := Trim(S);
...
class function TUniBaseAutoUpdate.IsNewFormatJson(ARoot: TJSONObject): Boolean;
begin
  if ARoot = nil then
    Exit(False);
...
class procedure TUniBaseAutoUpdate.ResetUpdateInfo(out Info: TUpdateInfo;
...
class procedure TUniBaseAutoUpdate.ParseInstallPolicy(Source: TJSONObject;
...
function TUniBaseAutoUpdate.CheckForUpdateFromNewFormat(
...
function TUniBaseAutoUpdate.CheckForUpdateFromLegacyFormat(
  ARoot: TJSONObject; out Info: TUpdateInfo): Boolean;
var
  Source: TJSONObject;
...
function TUniBaseAutoUpdate.CheckForUpdateFromJson(out Info: TUpdateInfo): Boolean;
var
  Client: THTTPClient;
...
function TUniBaseAutoUpdate.CheckForUpdateFromGitHub(
...
function TUniBaseAutoUpdate.CheckForUpdateFromGitee(
  const RepoSlug: string; out Info: TUpdateInfo): Boolean;
var
  Client: THTTPClient;
  Response: IHTTPResponse;
...
function TUniBaseAutoUpdate.CheckForUpdate(out Info: TUpdateInfo): Boolean;
var
  Prefix, Slug: string;
begin
  Result := False;
...
function TUniBaseAutoUpdate.CheckForUpdateAsync(
...
function TUniBaseAutoUpdate.DownloadUpdate(const Info: TUpdateInfo; const DestFile: string;
