unit UniBase.LLM.Config;

{ UniBase LLM Config — UniBase.Config 集成 + API Key 安全存储 }
...
type
  ...
  public
    ...
    // Persistence via UniBase.Config
    procedure Save;
    procedure Load;
...
begin
  ...
  try
    Result := TSimpleCrypto.Decrypt(AEncrypted, '@UniBase.LLM.Key');
  except
    Result := '';
  end;
end;
...
begin
  if APlain = '' then Exit('');
  Result := TSimpleCrypto.Encrypt(APlain, '@UniBase.LLM.Key');
end;
...
begin
  FTierModels.AddOrSetValue(ATier, AModels);
end;

function TLLMConfigStore.GetTierModels(const ATier: string): TArray<string>;
begin
  if not FTierModels.TryGetValue(ATier, Result) then
    SetLength(Result, 0);
end;
...
begin
  ...
  // Vision tiers (vision, vision_fallback) are optional
  // — only validated when ChatVision is actually called
...
var
  JSON, ProvObj, ModelsObj: TJSONObject;
  ProvArr, ModelsArr: TJSONArray;
  KeysObj: TJSONObject;
...
begin
  ...
  if TiersStr <> '' then
  begin
    JSON := TJSONObject.ParseJSONValue(TiersStr) as TJSONObject;
    if JSON <> nil then
    try
      ...
      begin
        ...
        begin
          ...
          FTierModels.AddOrSetValue(Pair.JsonString.Value, Models);
        end;
      end;
    finally
      JSON.Free;
