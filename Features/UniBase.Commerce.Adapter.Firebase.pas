unit UniBase.Commerce.Adapter.Firebase;

{==============================================================================
  UniBase.Commerce.Adapter.Firebase - Firebase Firestore Storage Adapter
...
class function TFirebaseConfig.Create(const AProjectId, AAccessToken: string;
  const ACollectionPrefix: string; ATimeoutMs: Integer): TFirebaseConfig;
begin
...
begin
  ...
  if Response.StatusCode >= 300 then
    raise EUniBaseCommerceError.CreateFmt('Firestore GET %s: %d',
...
begin
  ...
  try
    Response := FClient.Patch(
      AUrl + '?updateMask.fieldPaths=', BodyStream, AuthHeaders);
  finally
    BodyStream.Free;
  end;
  if Response.StatusCode >= 300 then
    raise EUniBaseCommerceError.CreateFmt('Firestore PATCH: %d', [Response.StatusCode]);
  Result := TJSONObject.ParseJSONValue(Response.ContentAsString(TEncoding.UTF8)) as TJSONObject;
end;
...
begin
  ...
  if Response.StatusCode >= 300 then
    raise EUniBaseCommerceError.CreateFmt('Firestore POST: %d', [Response.StatusCode]);
  Result := TJSONObject.ParseJSONValue(Response.ContentAsString(TEncoding.UTF8)) as TJSONObject;
end;
