{ ============================================================================
  UniBase.Unlock - Lightweight Unlock Module
  ...
  Description:
    ...
    separate from the full enterprise license system (UniBase.License).
...
const
  ...
  UNLOCK_SECRET = 'UniBase-Unlock-Seed-2025';
...
constructor TUniBaseUnlock.Create(const AProductCode: string);
begin
  inherited Create;
  FProductCode := NormalizeProductCode(AProductCode);
  if FProductCode = '' then
    raise EArgumentException.Create('ProductCode must not be empty for TUniBaseUnlock');

  FConfigKeyLevel := Format('Unlock.%s.Level', [FProductCode]);
  FConfigKeyCode  := Format('Unlock.%s.Code', [FProductCode]);
end;

class function TUniBaseUnlock.NormalizeProductCode(const Value: string): string;
begin
  Result := UpperCase(Trim(Value));
end;

class function TUniBaseUnlock.LevelToStr(Level: TUnlockLevel): string;
begin
  case Level of
    ulFree:   Result := 'Free';
...
class function TUniBaseUnlock.StrToLevel(const S: string): TUnlockLevel;
var
  Upper: string;
begin
...
class function TUniBaseUnlock.StatusToStr(Status: TUnlockValidationStatus): string;
begin
  case Status of
    uvsOk:              Result := 'Ok';
...
class function TUniBaseUnlock.UnlockLevelToStr(Level: TUnlockLevel): string;
begin
  Result := LevelToStr(Level);
end;

class function TUniBaseUnlock.UnlockStatusToStr(Status: TUnlockValidationStatus): string;
begin
  Result := StatusToStr(Status);
end;

function TUniBaseUnlock.GetStoredLevel: TUnlockLevel;
var
  S: string;
begin
  // Default to Free if nothing stored.
  S := UniBase.Config.GetConfig(FConfigKeyLevel, 'Free');
  Result := StrToLevel(S);
end;

procedure TUniBaseUnlock.SetStoredLevel(const Value: TUnlockLevel);
begin
  UniBase.Config.SetConfig(FConfigKeyLevel, LevelToStr(Value), UNLOCK_CONFIG_CATEGORY);
end;

function TUniBaseUnlock.GetStoredCode: string;
begin
  Result := UniBase.Config.GetConfig(FConfigKeyCode, '');
end;

procedure TUniBaseUnlock.SetStoredCode(const Value: string);
begin
  UniBase.Config.SetConfig(FConfigKeyCode, Trim(Value), UNLOCK_CONFIG_CATEGORY);
end;

class function TUniBaseUnlock.ComputeCheckChar(const AProductCode: string;
...
class function TUniBaseUnlock.ParseCode(const AProductCode, Code: string;
  out Year2, Month: Integer; out CheckChar: Char;
...
function TUniBaseUnlock.ValidateCode(const Code: string; out Info: TUnlockInfo): TUnlockValidationStatus;
var
  Y2, M: Integer;
  Check: Char;
  Status: TUnlockValidationStatus;
...
function TUniBaseUnlock.ApplyCode(const Code: string; out Info: TUnlockInfo): Boolean;
var
  Status: TUnlockValidationStatus;
...
class function TUniBaseUnlock.GenerateCode(const AProductCode: string; ADate: TDateTime;
