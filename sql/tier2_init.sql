...
-- ============================================================================
--
-- DeepBase Tier 2 Schema 初始化脚�?(已废�?
-- 版本: 0.3
-- 说明: 创建扩展功能表结构：LLM, Exception, Animation, Test
-- 依赖: tier0_init.sql, tier1_init.sql 必须先执�?
...
CREATE TABLE IF NOT EXISTS LLMConfiguration (
  ...
  Provider TEXT NOT NULL DEFAULT 'openai', -- 服务�?(openai, anthropic, litellm, azure, ollama)
  BaseUrl TEXT,                          -- API 基础 URL (LiteLLM: http://localhost:4000)
  ApiKey TEXT,                           -- API Key (加密存储)
  Model TEXT NOT NULL DEFAULT 'gpt-4o-mini', -- 模型名称
  MaxTokens INTEGER DEFAULT 4096,        -- 最大输�?Token
  Temperature REAL DEFAULT 0.7,          -- 温度参数 (0.0-2.0)
  SystemPrompt TEXT,                     -- 系统提示�?
  -- 成本配置
  InputTokenPrice REAL DEFAULT 0.0,      -- �?1K 输入 Token 价格 (USD)
  OutputTokenPrice REAL DEFAULT 0.0,     -- �?1K 输出 Token 价格 (USD)
  -- 状�?
  IsEnabled INTEGER DEFAULT 1,           -- 是否启用
  IsDefault INTEGER DEFAULT 0,           -- 是否为默认配�?
...
CREATE TABLE IF NOT EXISTS LLMCalls (
  Id INTEGER PRIMARY KEY AUTOINCREMENT,
  ConfigName TEXT NOT NULL,              -- 使用的配置名�?
  Provider TEXT NOT NULL,                -- 服务�?
  ...
  -- Token 统计
  InputTokens INTEGER DEFAULT 0,         -- 输入 Token �?
  OutputTokens INTEGER DEFAULT 0,        -- 输出 Token �?
  TotalTokens INTEGER DEFAULT 0,         -- �?Token �?
  -- 成本计算
...
CREATE TABLE IF NOT EXISTS AnimationAssets (
  ...
  FilePath TEXT,                         -- 或外部文件路�?
  -- 动画参数
  FrameCount INTEGER DEFAULT 1,          -- 帧数 (SVG 动画)
  FrameDuration INTEGER DEFAULT 100,     -- 帧间�?(毫秒)
...
CREATE TABLE IF NOT EXISTS TestSnapshots (
  ...
  Theme TEXT,                            -- 当前主题
  -- 版本
  Version INTEGER DEFAULT 1,             -- 快照版本�?
  IsBaseline INTEGER DEFAULT 0,          -- 是否为基线快�?
  -- 时间�?
  CreatedAt TEXT DEFAULT (datetime('now', 'localtime')),
  UpdatedAt TEXT DEFAULT (datetime('now', 'localtime')),
  Extra TEXT,                            -- 额外信息 (JSON)
...
CREATE TABLE IF NOT EXISTS LLMPromptTemplates (
  ...
  -- 模板内容
  SystemPrompt TEXT,                     -- 系统提示�?
  UserPromptTemplate TEXT NOT NULL,      -- 用户提示词模�?(支持 {{variable}})
  -- 参数
  Variables TEXT,                        -- 变量列表 (JSON: ["source_lang", "target_lang"])
  DefaultValues TEXT,                    -- 默认�?(JSON: {"source_lang": "en"})
  -- 推荐配置
  RecommendedConfig TEXT,                -- 推荐�?LLM 配置�?
  RecommendedModel TEXT,                 -- 推荐的模�?
  MaxTokens INTEGER,                     -- 推荐的最�?Token
  Temperature REAL,                      -- 推荐的温�?
  -- 状�?
