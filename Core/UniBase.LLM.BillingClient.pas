...
unit UniBase.LLM.BillingClient;
...
type
  ...
  /// <summary>
  /// Chat history manager - maintains conversation context
  /// </summary>
  TChatHistory = class
  private
  ...
  public
    ...
    /// <summary>Chat with message history</summary>
...
begin
  ...
  case StatusCode of
    401:
      raise EBillingAuthError.Create('认证失败: ' + ErrorMsg, StatusCode, ErrorCode);
    402:
      raise EBillingBalanceError.Create('余额不足: ' + ErrorMsg, StatusCode, ErrorCode);
    429:
      raise EBillingRateLimitError.Create('请求过于频繁: ' + ErrorMsg, StatusCode, ErrorCode);
    500..599:
      raise EBillingServerError.Create('服务器错误: ' + ErrorMsg, StatusCode, ErrorCode);
  else
...
begin
  ...
  try
    SetupHeaders;
    // 注意: 如果 BaseURL 已包含 /v1，则直接用 /chat/completions
    // 如果 BaseURL 是根路径，则用 /v1/chat/completions
...
begin
  ...
  raise EBillingError.Create('重试 ' + IntToStr(Retries) + ' 次后失败: ' + LastError);
end;
