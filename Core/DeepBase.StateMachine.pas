unit DeepBase.StateMachine;

(*******************************************************************************
  DeepBase State Machine
...
type
  ...
  /// <summary>State hiDeepStory entry</summary>
...
begin
  ...
  begin
    if TEqualityComparer<TTrigger>.Default.Equals(LTransition.Trigger, ATrigger) then
    begin
      LTransition.Action := AAction;
      Break;
    end;
...
destructor TStateMachineBuilder<TState, TTrigger>.Destroy;
begin
  FStateMachine.Free;
  inherited;
end;
...
begin
  // Generic state spaces cannot be enumerated reliably. A transition target does
  // not need its own configuration unless it has entry/exit actions or children.
  Result := True;
end;
