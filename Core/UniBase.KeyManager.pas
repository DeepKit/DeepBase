...
begin
  ...
  try
    AES.SetKey(AKEK);
    AES.GenerateIV;
    FEncryptedKeyData := AES.Encrypt(FKeyData);
  finally
    AES.Free;
  end;
end;
...
begin
  ...
  try
    AES.SetKey(AKEK);
    FKeyData := AES.Decrypt(FEncryptedKeyData);
  finally
    AES.Free;
  end;
end;
    ...
    try
      if FInstance = nil then
        FInstance := TKeyManager.Create(
          TPath.Combine(TPath.GetHomePath, '.unibase_keys.json'));
    finally
      FInstanceLock.Leave;
    end;
  end;
