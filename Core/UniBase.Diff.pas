...
begin
  // Don't free FHunk - it's owned by the patch
  inherited;
end;
...
var
  LLines1, LLines2: TArray<string>;
  LLCS: TArray<TArray<Integer>>;
  LLCSLen, LMaxLen: Integer;
begin
  ...
  LLines1 := Default.SplitLines(AText1);
  LLines2 := Default.SplitLines(AText2);
  
  LLCS := Default.ComputeLCS(LLines1, LLines2);
  LLCSLen := LLCS[Length(LLines1)][Length(LLines2)];
  LMaxLen := Max(Length(LLines1), Length(LLines2));
