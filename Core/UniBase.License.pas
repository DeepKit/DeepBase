{ ============================================================================
  UniBase.License - License Management Module
...
const
  LICENSE_SECRET = 'UniBase-License-2024';
...
function TUniBaseLicense.GetDeviceId: string;
begin
...
function TUniBaseLicense.EncodePayload(const Payload: string): string;
var
  Bytes: TBytes;
begin
...
function TUniBaseLicense.DecodePayload(const Encoded: string): string;
var
  Bytes: TBytes;
begin
  try
...
function TUniBaseLicense.SignData(const Data: string): string;
var
  Combined: string;
begin
...
function TUniBaseLicense.VerifySignature(const Data, Signature: string): Boolean;
begin
  Result := SameText(SignData(Data), Signature);
end;

class function TUniBaseLicense.GenerateLicenseKey(
  LicenseType: TLicenseType;
  ExpiresAt: TDateTime;
  const IssuedTo: string;
  const DeviceId: string;
...
begin
  ...
  // Encode and sign
  TempLicense := TUniBaseLicense.Create(nil);
  try
...
function TUniBaseLicense.ValidateLicense(const LicenseKey: string): TLicenseInfo;
var
  Parts: TArray<string>;
...
function TUniBaseLicense.ActivateLicense(const LicenseKey: string): Boolean;
var
  Info: TLicenseInfo;
begin
...
procedure TUniBaseLicense.DeactivateLicense;
begin
  FCurrentLicense := TLicenseInfo.Empty;
...
function TUniBaseLicense.IsFeatureLicensed(const FeatureName: string): Boolean;
begin
...
class function TUniBaseLicense.StrToLicenseType(const S: string): TLicenseType;
var
  Upper: string;
begin
...
class function TUniBaseLicense.LicenseStatusToStr(Status: TLicenseStatus): string;
begin
  case Status of
