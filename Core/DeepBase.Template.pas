...
type
  ...
  private
    ...
    FParentContext: TTemplateContext;
    function GetValues: TDictionary<string, Variant>;
    function GetParent: ITemplateContext;
  public
...
type
  TTemplateContextParentRef = class(TInterfacedObject, ITemplateContext)
  private
    FContext: TTemplateContext;
  public
    constructor Create(AContext: TTemplateContext);
    function GetValue(const AName: string): Variant;
    function HasValue(const AName: string): Boolean;
    function GetValues: TDictionary<string, Variant>;
    function GetParent: ITemplateContext;
...
{ TTemplateContextParentRef }

constructor TTemplateContextParentRef.Create(AContext: TTemplateContext);
begin
  inherited Create;
  FContext := AContext;
end;

function TTemplateContextParentRef.GetValue(const AName: string): Variant;
begin
  Result := FContext.GetValue(AName);
end;

function TTemplateContextParentRef.HasValue(const AName: string): Boolean;
begin
  Result := Assigned(FContext) and FContext.HasValue(AName);
end;

function TTemplateContextParentRef.GetValues: TDictionary<string, Variant>;
begin
  if Assigned(FContext) then
    Result := FContext.Values
  else
    Result := nil;
end;

function TTemplateContextParentRef.GetParent: ITemplateContext;
begin
  if Assigned(FContext) then
    Result := FContext.Parent
  else
    Result := nil;
end;

procedure TTemplateContextParentRef.SetValue(const AName: string; const AValue: Variant);
begin
  if Assigned(FContext) then
    FContext.SetValue(AName, AValue);
end;
...
begin
  if Assigned(FParentContext) then
    Result := TTemplateContextParentRef.Create(FParentContext)
  else
    Result := FParent;
end;
...
begin
  if FValues.TryGetValue(AName, Result) then
    Exit;
  if Assigned(FParentContext) then
    Result := FParentContext.GetValue(AName)
...
begin
  ...
  if (not Result) and Assigned(FParentContext) then
    Result := FParentContext.HasValue(AName)
  else if (not Result) and Assigned(FParent) then
...
begin
  ...
  begin
    ...
    while (not LIsRaw) and
          (Copy(FTemplate, FPosition + Length(FEndDelimiter), Length(FEndDelimiter) + 1) = ' ' + FEndDelimiter) do
    begin
      LContent := LContent + FEndDelimiter;
      Read(Length(FEndDelimiter));
      LContent := LContent + ReadUntil(FEndDelimiter);
    end;
    if LIsRaw then
      Read(3)
    else
...
var
  I: Integer;
  LNode, LNextNode: TTemplateNode;
begin
  Result := '';
  for I := 0 to ANodes.Count - 1 do
  begin
    LNode := ANodes[I];
    if LNode.NodeType = ntComment then
    begin
      if I < ANodes.Count - 1 then
      begin
        LNextNode := ANodes[I + 1];
        if (Result <> '') and (Result[Length(Result)] = ' ') and
           (LNextNode.NodeType = ntText) and (LNextNode.Content <> '') and
           (LNextNode.Content[1] = ' ') then
...
begin
  ...
  LChildContext := TTemplateContext.Create;
  try
    LChildContext.AddDictionary(AContext.Values);
    LIndex := 0;
...
begin
  ...
  if AContext.HasValue(APath) then
    Exit(AContext.GetValue(APath));

  if (Pos('.', APath) = 0) and (Pos('[', APath) = 0) then
  begin
    if FStrictMode then
...
begin
  ...
  begin
    LColonPos := Pos(':', AFilter);
    if LColonPos > 0 then
    begin
      AFilterName := Trim(Copy(AFilter, 1, LColonPos - 1));
      LArgsStr := Copy(AFilter, LColonPos + 1);
      LArgStrings := LArgsStr.Split([':']);
      ParseArgs;
    end
    else
