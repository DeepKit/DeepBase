{ ============================================================================
  UniBase.Plugin - Plugin Interfaces and Types
  
  Version: 0.3
  Description: Defines the plugin interface contract and supporting types
               for the UniBase plugin system.
  
  Plugin developers should implement IUniBasePlugin interface and export
...
type
  ...
  TPluginInfo = record
    ...
    /// <summary>Minimum required UniBase version</summary>
    MinUniBaseVersion: string;
  ...
  /// </summary>
  IUniBasePluginEvents = interface
  ...
  /// </summary>
  IUniBasePluginContext = interface
  ...
  /// </summary>
  TUniBasePluginBase = class(TInterfacedObject, IUniBasePlugin)
  private
    FState: TPluginState;
    FLastError: string;
  ...
  protected
    ...
    property Context: IUniBasePluginContext read FContext;
  ...
  public
    ...
    procedure SetContext(AContext: IUniBasePluginContext);
    
    // IUniBasePlugin implementation
    function GetPluginInfo: TPluginInfo;
    function Initialize: Boolean;
    function Finalize: Boolean;
    function GetState: TPluginState;
...
/// </summary>
type
  TRegisterPluginFunc = function: IUniBasePlugin;
...
function MakePluginInfo(
  ...
  const AMinUniBaseVersion: string = '0.3';
...
function MakePluginInfo(
  ...
  const AName, AVersion, AAuthor, ADescription: string;
  ACapabilities: TPluginCapabilities;
  const AMinUniBaseVersion: string;
...
begin
  ...
  Result.MinUniBaseVersion := AMinUniBaseVersion;
...
constructor TUniBasePluginBase.Create;
begin
  inherited Create;
  FState := psUnloaded;
  FLastError := '';
...
procedure TUniBasePluginBase.SetContext(AContext: IUniBasePluginContext);
begin
  FContext := AContext;
end;

procedure TUniBasePluginBase.SetError(const ErrorMsg: string);
begin
  FLastError := ErrorMsg;
  FState := psError;
end;

procedure TUniBasePluginBase.SetState(AState: TPluginState);
begin
  FState := AState;
end;

function TUniBasePluginBase.GetPluginInfo: TPluginInfo;
begin
  Result := DoGetPluginInfo;
end;

function TUniBasePluginBase.Initialize: Boolean;
begin
  FState := psLoading;
  FLastError := '';
...
function TUniBasePluginBase.Finalize: Boolean;
begin
  FState := psUnloading;
...
function TUniBasePluginBase.DoInitialize: Boolean;
begin
  // Default implementation - override in derived classes
  Result := True;
end;

function TUniBasePluginBase.DoFinalize: Boolean;
begin
  // Default implementation - override in derived classes
  Result := True;
end;

function TUniBasePluginBase.GetState: TPluginState;
begin
  Result := FState;
end;

function TUniBasePluginBase.GetLastError: string;
begin
  Result := FLastError;
end;
