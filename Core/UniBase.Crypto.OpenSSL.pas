{ ============================================================================
  UniBase.Crypto.OpenSSL - OpenSSL libcrypto Backend for Cross-Platform Encryption
...
begin
  ...
  // Build search paths
  EnvPath := GetEnvironmentVariable('UNIBASE_LIBCRYPTO_PATH');
  if EnvPath <> '' then
    SearchPaths := [EnvPath]
  else
  begin
  ...
  try
    ...
    if not TryLoadLibrary then
      raise EOpenSSLNotLoaded.Create(
        ...
        'UNIBASE_LIBCRYPTO_PATH environment variable.'
        {$ELSE}
