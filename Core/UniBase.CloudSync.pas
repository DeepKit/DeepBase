unit UniBase.CloudSync;

{*******************************************************************************
  UniBase Framework - Cloud Configuration Sync
...
begin
  ...
  LVersionObj := AJSON.GetValue<TJSONObject>('localVersion');
  if Assigned(LVersionObj) then
    Result.FLocalVersion := TConfigVersion.FromJSON(LVersionObj);
    
  LVersionObj := AJSON.GetValue<TJSONObject>('remoteVersion');
  if Assigned(LVersionObj) then
    Result.FRemoteVersion := TConfigVersion.FromJSON(LVersionObj);
end;
...
begin
  Result.ServiceURL := 'https://api.unibase.cloud/v1';
  Result.ApiKey := '';
...
var
  LBytes: TBytes;
begin
  // 简化的AES加密实现（实际使用应该用完整的AES-256-GCM）
  // 这里用Base64编码模拟，实际项目中应使用真正的加密库
