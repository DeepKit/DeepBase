{ ============================================================================
  DeepBase.IoC - Inversion of Control / Dependency Injection Container
...
type
  ...
  /// <summary>
  /// Non-generic interface factory function
  /// </summary>
  TServiceInterfaceFactoryFunc = reference to function(Container: TIoCContainer): IInterface;
  ...
  private
    ...
    FInterfaceFactory: TServiceInterfaceFactoryFunc;
    FSingletonInstance: TObject;
    FSingletonInterface: IInterface;
    FOwnsInstance: Boolean;
    FHasSingletonInterface: Boolean;
    FName: string;
  public
    ...
    property InterfaceFactory: TServiceInterfaceFactoryFunc read FInterfaceFactory write FInterfaceFactory;
    property SingletonInstance: TObject read FSingletonInstance write FSingletonInstance;
    property SingletonInterface: IInterface read FSingletonInterface write FSingletonInterface;
    property OwnsInstance: Boolean read FOwnsInstance write FOwnsInstance;
    property HasSingletonInterface: Boolean read FHasSingletonInterface write FHasSingletonInterface;
  ...
  private
    ...
    FScopedInterfaces: TDictionary<PTypeInfo, IInterface>;
    FDisposed: Boolean;
  public
...
begin
  ...
  FScopedInterfaces := TDictionary<PTypeInfo, IInterface>.Create;
  FDisposed := False;
end;
...
procedure TIoCScope.Dispose;
begin
  if FDisposed then
    Exit;
  FDisposed := True;
  FreeAndNil(FScopedInterfaces);
...
begin
  ...
  begin
    Intf := FParent.ResolveInterfaceInternal(ServiceTypeInfo, Self);
    TypeData := GetTypeData(ServiceTypeInfo);
    if Assigned(Intf) and (Intf.QueryInterface(TypeData^.GUID, Result) = S_OK) then
      Exit;
    raise EIoCException.CreateFmt('Cannot cast to interface: %s', [GetTypeName(ServiceTypeInfo)]);
  end;

  Instance := Resolve(ServiceTypeInfo);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ParamType^.Kind = tkInterface then
        begin
          ParamInterface := ResolveInterfaceInternal(ParamType, Scope);
          TValue.Make(@ParamInterface, ParamType, Args[I]);
        end
        else
        begin
...
function TIoCContainer.ResolveInterfaceInternal(ServiceType: PTypeInfo;
  Scope: TIoCScope; const Name: string): IInterface;
var
  Reg: TServiceRegistration;
  Instance: TObject;
  TypeData: PTypeData;
  ...
  function CreateObjectBackedInterface: IInterface;
  begin
    if FInterceptors.Count > 0 then
    begin
      Context := TInterceptorContext.Create(ServiceType, Reg.ImplementationType, nil);
      ApplyInterceptors(Context, True);
    end;

    Instance := CreateInstance(Reg, Scope);

    if FInterceptors.Count > 0 then
    begin
      Context := TInterceptorContext.Create(ServiceType, Reg.ImplementationType, Instance);
      ApplyInterceptors(Context, False);
      if Context.Instance <> Instance then
        Instance := Context.Instance;
    end;

    TypeData := GetTypeData(ServiceType);
    if not Instance.GetInterface(TypeData^.GUID, Result) then
      raise EIoCException.CreateFmt('Cannot cast to interface: %s', [GetTypeName(ServiceType)]);
  end;
begin
  Result := nil;
    ...
    case Reg.Lifetime of
      slTransient:
        begin
          if Assigned(Reg.InterfaceFactory) then
            Result := Reg.InterfaceFactory(Self)
          else
            Result := CreateObjectBackedInterface;
        end;
      ...
      slScoped:
        begin
          if Scope = nil then
            raise EIoCException.Create('Scoped service requires a scope. Use CreateScope.');
          if Scope.FDisposed then
            raise EScopeDisposedException.Create('Cannot resolve from disposed scope');

          if not Scope.FScopedInterfaces.TryGetValue(ServiceType, Result) then
          begin
            if Assigned(Reg.InterfaceFactory) then
              Result := Reg.InterfaceFactory(Self)
            else
              Result := CreateObjectBackedInterface;
            Scope.FScopedInterfaces.Add(ServiceType, Result);
          end;
        end;
    end;
  finally
...
begin
  ...
  if ServiceTypeInfo^.Kind = tkInterface then
  begin
    Intf := ResolveInterfaceInternal(ServiceTypeInfo, nil);
    TypeData := GetTypeData(ServiceTypeInfo);
    if (not Assigned(Intf)) or (Intf.QueryInterface(TypeData^.GUID, Result) <> S_OK) then
      raise EIoCException.CreateFmt('Cannot cast to interface: %s', [GetTypeName(ServiceTypeInfo)]);
  end
  else
  begin
...
begin
  ...
  if ServiceTypeInfo^.Kind = tkInterface then
  begin
    Intf := ResolveInterfaceInternal(ServiceTypeInfo, nil, Name);
    TypeData := GetTypeData(ServiceTypeInfo);
    if (not Assigned(Intf)) or (Intf.QueryInterface(TypeData^.GUID, Result) <> S_OK) then
      raise EIoCException.CreateFmt('Cannot cast to interface: %s', [GetTypeName(ServiceTypeInfo)]);
  end
