...
{*******************************************************************************
  ...
  - 保留旧 DatabaseType/ConnectionString 兼容入口
  - 连接健康检查
  - 空闲连接回收
  - 连接泄漏检测
  - 详细统计监控
...
type
  ...
  TDBPoolManagerRuntimeComponent = class(TUniBaseRuntimeComponent)
  private
    FStarted: Boolean;
  public
...
begin
  ...
  try
    ...
    begin
      {$IFDEF DEBUG}
      OutputDebugString(PChar('UniBase.DB.Pool: Warmup connection creation failed: ' + E.Message));
      {$ENDIF}
