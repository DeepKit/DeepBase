...
## 2026-08-24 全库审查修复战役（doQry / Persistence / Core，约109K 行）?

> 来源: docs/code-review-2026-08-24.md（报告）+ docs/code-review-2026-08-24-tasks.md（跟踪清单，含十一批修复记录）
> 提交链: 92c10a6 → 7ddfbed → c9443b1 → c8dbb53 → 175d00f → e249883 → acf64be → 1b1df35
> 规模: P0 18/18、?? 约30项、Owner 决策 4/4、系统性主题 4 大项全破

### 批次一览

- **批次一~二**: P0 数据丢失/安全 + 崩溃/UAF（KeyManager 盐持久化、JobQueue PG 出队 SQL、ORM 参数错位、doQry 注入与全表删除、连接池 ABBA 死锁等）
- **批次三**: Serialization record/locale/白名单/枚举四连 + Feedback/Configuration/FeatureFlags 崩溃族 + FileWatcher 三连
- **批次四**: 二进制反序列化加固 + Diff 算法栈溢出/内存爆炸 + 模板比较运算失效
- **批次五**: 遗留项清理（CR-605 基准校准、CR-606 排查归档）
- **批次六**: ?? 快赢十项（强度量纲/TTL 符号/负计数/时间取整/保留字/DST/CTE/UTC 日志等）
- **批次七**: SQLLogger 方言+ensured、Diagnose 版本比较、EventBus 回调防护、限流器构造校验、ISO Try 契约、军区 J、四单元异常基类挂 EDeepBaseException
- **批次八**: 查询缓存多库键、ExecuteFirst Limit 污染、Logger 析构排干、毒丸 DLQ
- **批次九**: Diff hunk 尾随上下文/SideBySide 行号、i18n 缓存 TOCTOU、Schema 键常量化、Math.Random 平台守卫
- **批次十**: CR-290 单调时钟——限流四算法+熔断器 34 处墙钟计时全面切换 GetTickCount64
- **批次十一**: Owner 四项决策落地（fail-closed / WithLevel 追加 / 序列化空根快速失败 / App.LogLevel 幂等迁移）

### 新增基础设施

- 回归测试单元 ×6: Tests\Regression\Test.Regression.CR20260824_{P0Batch1..P0Batch5,OwnerDecisions}.pas（25 用例）
- 编译验证工具: Scripts\verify_doqry.ps1 + Tools\DBClientStub\DBClient.pas（解锁无 MIDAS 机器的 doQry 编译验证）
- 文档: docs\code-review-2026-08-24.md（审查报告）

### 未尽事项

见 tasks.md「待办」：7 条 P0 ?? 待环境验证转正、CR-606 Perception 图形会话复跑、CR-605 性能门禁迁专用 runner、?? 部分子项与 ?? 长尾。
...
- [x] **REVIEW5-R2-CORE-011** (CORE-R2-011 / BUG-372): `Core/DeepBase.Metrics.pas` TSummary.Observe O(n2) 清理 — 改固定容量环形缓冲, 写入 O(1) ?
...
- Mojibake 检测: UTF-8 BOM 误读 (`???`/`?ˉ????`)、GBK 双编码常见 CJK 碎片 (`?????`=确定, `?3é`=取消 等)、CP1252-as-UTF-8 通用签名 (`?` + 高字节 ×3+)
