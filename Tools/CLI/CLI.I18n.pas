...
begin
  ...
  Writeln('');
  Writeln('Usage: unibase i18n <subcommand> [options]');
  Writeln('');
  Writeln('Subcommands:');
  ...
  Writeln('  --api-key <key>       LLM API key (or set UNIBASE_LLM_KEY env var)');
  Writeln('');
  ...
  Writeln('');
  Writeln('Examples:');
  Writeln('  unibase i18n scan --path ./src --output strings.json -r');
  Writeln('  unibase i18n sync --source strings.json --target zh-CN.json');
  Writeln('  unibase i18n translate -i en.json -o zh-CN.json --to zh-CN');
  Writeln('  unibase i18n export -i strings.json -o messages.po -f po');
end;
...
begin
  ...
  ApiKey := TCliUtils.GetOption('api-key', GetEnvironmentVariable('UNIBASE_LLM_KEY'));
  
  if ApiKey = '' then
  begin
    TCliUtils.Error('LLM API key is required. Use --api-key or set UNIBASE_LLM_KEY environment variable.');
    Result := 1;
    Exit;
  end;
  ...
  TCliUtils.Info('Please use UniBase Studio for LLM-based translation.');
  TCliUtils.Info('');
  TCliUtils.Info('Alternatively, you can:');
  TCliUtils.Info('  1. Export to PO format: unibase i18n export -i %s -o messages.po -f po', [InputFile]);
  TCliUtils.Info('  2. Use external translation tools (e.g., Poedit, Crowdin)');
  TCliUtils.Info('  3. Import back: unibase i18n import -i translated.po -o %s', [OutputFile]);
...
begin
  ...
  try
    ...
    begin
      ...
      try
        Output.Add('# UniBase Translation File');
        Output.Add('# Generated by UniBase CLI');
        Output.Add('');
        Output.Add('msgid ""');
        Output.Add('msgstr ""');
