{ ============================================================================
  DeepBase.CLI.SSH - SSH Remote Execution Support
...
begin
  ...
  begin
    CloseBracketPos := Pos(']', HostPart);
    if CloseBracketPos > 1 then
    begin
      Host := Copy(HostPart, 2, CloseBracketPos - 2);
      if (Length(HostPart) > CloseBracketPos) and
         (HostPart[CloseBracketPos + 1] = ':') then
        TryStrToInt(Copy(HostPart, CloseBracketPos + 2, MaxInt), Port)
      else
        Port := 22;
    end
    else
  ...
  begin
    FirstColonPos := Pos(':', HostPart);
    LastColonPos := LastDelimiter(':', HostPart);
    if (FirstColonPos > 0) and (FirstColonPos = LastColonPos) then
    begin
      Host := Copy(HostPart, 1, FirstColonPos - 1);
      TryStrToInt(Copy(HostPart, FirstColonPos + 1, MaxInt), Port);
    end
    else
    begin
      Host := HostPart;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        on E: Exception do
          {$IFDEF DEBUG}
          OutputDebugString(PChar('DeepBase.CLI.SSH: Cleanup error: ' + E.Message));
          {$ENDIF}
      end;
    end
...
begin
  ...
  try
    ...
    on E: Exception do
      {$IFDEF DEBUG}
      OutputDebugString(PChar('DeepBase.CLI.SSH: LoadAliases parse error: ' + E.Message));
      {$ENDIF}
  end;
end;
