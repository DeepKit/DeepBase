unit DeepFlow.Message;

{*******************************************************************************
  DeepFlow.Message - 消息基类定义
  
  描述：
    定义 DeepFlow 角色间通信的消息基类和消息类型。
    所有角色间的通信都通过标准化消息进行。
...
type
  ...
  /// <summary>DeepFlow 消息基类</summary>
  TDeepFlowMessage = class
  private
    FMsgId: string;
  ...
  public
    ...
    class function FromJSON(const AJSON: TJSONObject): TDeepFlowMessage;
    /// <summary>克隆消息</summary>
    function Clone: TDeepFlowMessage;
  ...
  TRequestMessage = class(TDeepFlowMessage)
  private
    FTimeout: Integer;
  ...
  TResponseMessage = class(TDeepFlowMessage)
  private
    FSuccess: Boolean;
    FErrorCode: string;
    FErrorMessage: string;
  public
    constructor Create(const ARequest: TDeepFlowMessage);
  ...
  TEventMessage = class(TDeepFlowMessage)
  private
    FEventName: string;
  public
...
constructor TDeepFlowMessage.Create;
begin
  inherited Create;
...
constructor TDeepFlowMessage.Create(const AMsgType: string);
begin
  Create;
  FMsgType := AMsgType;
end;

destructor TDeepFlowMessage.Destroy;
begin
  FPayload.Free;
  inherited;
end;

function TDeepFlowMessage.ToJSON: TJSONObject;
begin
...
class function TDeepFlowMessage.FromJSON(const AJSON: TJSONObject): TDeepFlowMessage;
begin
  Result := TDeepFlowMessage.Create;
  // 解析基本字段
...
function TDeepFlowMessage.Clone: TDeepFlowMessage;
begin
  Result := TDeepFlowMessage.Create(FMsgType);
...
constructor TResponseMessage.Create(const ARequest: TDeepFlowMessage);
begin
