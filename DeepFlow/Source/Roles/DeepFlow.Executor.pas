unit DeepFlow.Executor;

{*******************************************************************************
  DeepFlow.Executor - 执行者角色 (L2 能力层)
...
type
  ...
  TExecutor = class(TDeepFlowRoleBase, IExecutor)
  private
...
constructor TExecutor.Create(const AConfig: TDeepFlowConfig);
var
  Meta: TRoleMetaInfo;
begin
  Meta.Name := 'Executor';
...
function TExecutor.DoHandleMessage(const AMessage: TDeepFlowMessage): TDeepFlowMessage;
var
  SkillName: string;
  Input: TJSONObject;
  Prompt: string;
