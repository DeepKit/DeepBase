{ ============================================================================
  DeepBase.VCL.AutoUpdater - 自动更新组件
  
  版本: 0.3
  说明: 非可视组件，封装 AutoUpdate 核心模块和 UI 交互
  ============================================================================ }

unit DeepBase.VCL.AutoUpdater;
...
begin
  ...
  // If URL is still empty, try to read from config when DeepBase is available
  if (FAutoUpdate.UpdateUrl = '') and DeepBase.Manager.DeepBase.IsInitialized then
    FAutoUpdate.UpdateUrl := DeepBase.Manager.DeepBase.Config.GetConfig('App.UpdateUrl', '');
