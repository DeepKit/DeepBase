{ ============================================================================
  DeepBase.VCL.LicenseStatusPanel - 许可证状态面板组件
  
  版本: 1.0
  功能: 显示当前许可证状态、类型、过期时间等信息
  ============================================================================ }

unit DeepBase.VCL.LicenseStatusPanel;
...
procedure Register;
begin
  RegisterComponents('DeepBase', [TLicenseStatusPanel]);
end;
...
begin
  ...
  if Info.IsValid then
    FLblStatus.Caption := 'Status: ' + TDeepBaseLicense.LicenseStatusToStr(Info.Status)
  else
    FLblStatus.Caption := 'Status: Not Licensed';
  
  // 类型
  FLblType.Caption := 'Type: ' + TDeepBaseLicense.LicenseTypeToStr(Info.LicenseType);
...
procedure TLicenseStatusPanel.SetLicense(Value: TDeepBaseLicense);
begin
