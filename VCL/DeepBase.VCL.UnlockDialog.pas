{ ============================================================================
  DeepBase.VCL.UnlockDialog - Lightweight Unlock Dialog
  
  Version: 0.1
  Description:
    VCL dialog for entering and applying lightweight unlock codes managed by
    TDeepBaseUnlock. Intended for simple "follow / share" flows where the user
...
unit DeepBase.VCL.UnlockDialog;
...
type
  ...
  public
    ...
    /// @param Unlock       Optional shared TDeepBaseUnlock instance; if nil a temporary
    ...
    class function Execute(const ProductCode: string; const QRCodePath: string = '';
      Unlock: TDeepBaseUnlock = nil): Boolean;
  end;
...
class function TUnlockDialog.Execute(const ProductCode: string; const QRCodePath: string;
  Unlock: TDeepBaseUnlock): Boolean;
var
  Dlg: TUnlockDialog;
begin
  Result := False;
  ...
  try
    ...
    begin
      Dlg.FUnlock := TDeepBaseUnlock.Create(ProductCode);
