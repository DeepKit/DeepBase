# UniBase Cloud Services

本目录包含 UniBase 框架云端服务所需的示例配置文件。
...
| sha256 | string | 文件 SHA256 校验和 |
| releaseNotes | string | 更新说明 |
| releaseDate | string | 发布日期 |
| isMandatory | boolean | 是否强制更新 |
| minOsVersion | string | 最低系统版本要求 |
...
|------|------|------|
| value | string | 配置值 |
| type | string | 值类型 (string/integer/boolean/float) |
...
| rolloutPercentage | integer | 灰度发布百分比 (0-100) |
...
**CDN 配置要点:**
1. 启用 HTTPS
2. 设置合理的缓存时间 (建议 5-15 分钟)
3. 启用 CORS
4. 启用 Gzip 压缩

### 3. 动态 API 服务

如果需要更复杂的逻辑（如用户分组、A/B测试），可以使用动态 API：
...
```pascal
uses UniBase.AutoUpdate, UniBase.VCL.AutoUpdater;
...
// 方式2: 直接使用类
var
  Updater: TUniBaseAutoUpdate;
begin
  Updater := TUniBaseAutoUpdate.Create('https://api.example.com/v1/version.json');
  try
...
```pascal
uses UniBase.RemoteConfig;

var
  Config: TUniBaseRemoteConfig;
begin
  Config := TUniBaseRemoteConfig.Create('https://api.example.com/v1/config.json');
  try
...
1. **始终使用 HTTPS**
2. **验证 SHA256 校验和** - 下载文件后验证完整性
3. **签名验证** - 可选：使用代码签名或 JSON 签名
4. **速率限制** - 防止滥用
5. **监控告警** - 监控配置变更和异常访问
