...
begin
  LabelWelcomeDesc.Text := '[Welcome] UniBase.Manager - Core manager singleton' + #13#10 +
    'Controls: TLayout, TLabel, TMemo, TTreeView, TTabControl' + #13#10 +
    'Functions: UniBase(), UNIBASE_VERSION, IsInitialized, RootPath';
  MemoWelcome.Lines.Clear;
  MemoWelcome.Lines.Add('=== UniBaseRun Demo Application ===');
  MemoWelcome.Lines.Add('');
  MemoWelcome.Lines.Add('UniBase Version: ' + UNIBASE_VERSION);
  ...
  MemoWelcome.Lines.Add('');
  MemoWelcome.Lines.Add('UniBase Modules Demonstrated:');
  MemoWelcome.Lines.Add('  1. UniBase.Manager   - Core manager, singleton pattern');
  MemoWelcome.Lines.Add('  2. UniBase.Config    - Config management, GetConfig/SetConfig');
  MemoWelcome.Lines.Add('  3. UniBase.i18n      - Internationalization, T()/TFmt()');
  MemoWelcome.Lines.Add('  4. UniBase.Logging   - Logging, Debug/Info/Warn/Error');
  MemoWelcome.Lines.Add('  5. UniBase.Types     - Common types and records');
  MemoWelcome.Lines.Add('  6. UniBase.FormState - Save/restore window position');
  MemoWelcome.Lines.Add('');
...
var
  ...
  i: Integer;
begin
  LabelConfigDesc.Text := '[Config] UniBase.Config - Configuration management' + #13#10 +
  ...
  begin
    ListBoxConfigGroups.Items.Add('--- UniBase.Config Demo ---');
...
var
  ...
  i: Integer;
begin
  LabelI18nDesc.Text := '[I18n] UniBase.i18n - Internationalization' + #13#10 +
  ...
  begin
    ...
    ListBoxLanguages.Items.Add('UniBase.I18n module not initialized.');
    ListBoxLanguages.Items.Add('');
    if not FUB.IsInitialized then
      ListBoxLanguages.Items.Add('Reason: UniBase init failed - ' + FUB.LastError)
    else
...
begin
  LabelToolsDesc.Text := '[Tools] UniBase.Manager - Health check' + #13#10 +
    'Controls: TLayout, TLabel, TButton, TMemo' + #13#10 +
    'Functions: HealthCheck, THealthCheckResult';
  MemoToolsOutput.Lines.Clear;
  MemoToolsOutput.Lines.Add('UniBase System Tools:');
  MemoToolsOutput.Lines.Add('');
  MemoToolsOutput.Lines.Add('1. Self Check - UniBase.Manager.HealthCheck()');
...
begin
  if TabControlMain.ActiveTab = TabItemWelcome then
    UpdateInfoPanel('Welcome', 'UniBase.Manager' + #13#10 + 'Core singleton')
  else if TabControlMain.ActiveTab = TabItemConfig then
    UpdateInfoPanel('Config', 'UniBase.Config' + #13#10 + 'Thread-safe cache')
  else if TabControlMain.ActiveTab = TabItemI18n then
    UpdateInfoPanel('I18n', 'UniBase.i18n' + #13#10 + 'T()/TFmt()/TN()')
  else if TabControlMain.ActiveTab = TabItemLogs then
    UpdateInfoPanel('Logs', 'UniBase.Logging' + #13#10 + 'Async queue')
...
begin
  MemoToolsOutput.Lines.Clear;
  MemoToolsOutput.Lines.Add('=== UniBase Health Check ===');
  MemoToolsOutput.Lines.Add('');
  
  MemoToolsOutput.Lines.Add('UniBase.IsInitialized: ' + BoolToStr(FUB.IsInitialized, True));
  MemoToolsOutput.Lines.Add('UniBase.LastError: ' + FUB.LastError);
  MemoToolsOutput.Lines.Add('UniBase.InitErrorCode: ' + IntToStr(Ord(FUB.InitErrorCode)));
  MemoToolsOutput.Lines.Add('');
  ...
  begin
    ...
    MemoToolsOutput.Lines.Add('[ERROR] UniBase not initialized');
