{ ============================================================================
  UniBase.FMX.MRUControls - FMX MRU 控件
  
  版本: 1.0
  说明: 自动绑定 MRU 列表的 FMX 控件
  控件:
    - TFMXMRUComboBox: MRU 下拉列表
  ============================================================================ }

unit UniBase.FMX.MRUControls;
...
begin
  ...
  MRUItems := UniBase.Manager.UniBase.MRU.GetMRUItems(FCategory, FMaxItems);
...
begin
  ...
  if (ItemIndex >= 0) and (Selected <> nil) then
    UniBase.Manager.UniBase.MRU.AddMRU(FCategory, Selected.Text, Selected.Text);
end;
...
begin
  ...
  if (ItemIndex >= 0) and UniBase.Manager.UniBase.IsInitialized then
  begin
    UniBase.Manager.UniBase.MRU.AddMRU(FCategory, GetSelectedKey, GetSelectedKey);
  end;
