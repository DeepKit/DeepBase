# 连续执行纪律（最高优先级 · 老板指令 2026-08-27）

已获批准的工单/任务范围内，开发期间不停等：不逐项等待人类确认、不中途停下等“继续”、进度一律以台账/PG Tag/工单状态留痕代替口头汇报；Commit 属授权范围动作可先行，Code Review 由主控承担。仅三类情形允许停等：H5/Human Gate 与不可逆操作、阻断性 blocker、重大战略歧义需老板裁定。本条与下文任何“等待人类确认后再开发”“经过人类允许后再 Git Commit”类条款冲突时，以本条为准。

适用分层：①**睿智工作流内**的开发按其完整链路执行（契约冻结→PG Tag→worker staging→SPW→integrate→精确提交）；②**睿智工作流之外**的常规项目 WP 开发遵守《非睿智轻量门禁》：WP 启动即落本地契约 brief（目标/范围/验收/digest）；build 与 test 的原始输出必须存档为证据文件（禁止仅自报 PASS 文本）；产物由异族模型或主控独立复核；存在未清债务一律禁止宣告 CLOSE。门禁的机器可查工具由 WO-20260827-0209 交付（`09_工程脚本/bgor/`），落地前按本条原则人工执行。

> 规范指针：详见《非睿智轻量门禁规范-v1》（`08_元管理/BCW/docs/非睿智轻量门禁规范-v1.md`），所有非睿智工作流之下的常规项目 WP 必须遵守五项硬门禁（G1–G5）。
...
* 尽量采用强类型设计。
* 生产级项目删除兼容/历史包袱前，须过兼容性评估（影响用户、数据迁移成本、替代路径）；内部工具可直接面向长期干净整洁。
* 密钥、凭据一律走环境变量/凭据管理器，禁止提交到仓库（含私有仓）。
...
* 注释只写 why（为什么这么做、业务/取舍/坑），不复述 what（代码做了什么）；复杂业务逻辑必须加注释。
* 涉及 PII/支付/许可/合规的代码，须在方案阶段标注风险，不得后补。
